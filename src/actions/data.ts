...
async function importAnimals(
  ...
  for (const [i, row] of rows.slice(1).entries()) {
    ...
    for (const col of cols) {
      /*
       * `uuid` пропускается вместе с номером и полом: он только ищет,
       * но не записывается. Ключ учётной системы выдаёт книга при
       * создании карточки и не меняет никогда — позволить файлу
       * проставить его значило бы дать чужому файлу назначить животному
       * личность. Для обратного файла реестра это к тому же бессмысленно:
       * там стоит тот же ключ, который мы сами и отдали.
       */
      if (col.key === 'identNumber' || col.key === 'sex' || col.key === 'uuid') continue
    ...
    try {
      /*
       * Существующее животное ищется сперва по нашему ключу, потом
       * по индивидуальному номеру.
       *
       * ## Зачем два поиска
       *
       * Обратный файл ФГИАС возвращается тем же шаблоном «Основные
       * сведения», который мы отдали, и животное в нём названо обоими
       * способами. Пока искали только по номеру, всё держалось на честном
       * слове реестра: сохранит он колонку УНЖ — файл ляжет, не сохранит —
       * не найдётся ни одна строка, и базовые номера не приедут никогда.
       *
       * Ключ надёжнее по устройству: номер хозяйство может перебить
       * при перемечивании, а ключ не меняется.
       *
       * ## Почему ключ первым
       *
       * Если в строке есть оба и они указывают на разные записи, верить
       * надо ключу: номер мог перебить кто угодно, ключ выдали мы сами
       * и не меняли. Случай редкий, но молчаливо выбрать номер значило бы
       * записать данные не тому животному.
       */
      const byKey = get('uuid')

      const found = byKey
        ? await payload.find({
            collection: 'animals',
            where: { uuid: { equals: byKey } },
            limit: 1,
            overrideAccess: true,
          })
        : null
      ...
      /*
       * Ключ не нашёлся — ищем по номеру, а не заводим новое животное.
       *
       * Так бывает у обратного файла, собранного до того, как карточку
       * завели: ключ в файле чужой или устаревший, а животное с таким
       * номером в книге есть. Создать второе значило бы получить дубль
       * там, где мы как раз боремся с дублями.
       */
      const existing =
        found && found.totalDocs > 0
          ? found
          : await payload.find({
              collection: 'animals',
