...
export async function registerLabProtocolAction(
  ...
  try {
     ...
     */
    /*
     * Про ступень говорится только тогда, когда она вправду изменилась.
     *
     * `syncTrustFromLab` не трогает записи, уже верифицированные
     * Ассоциацией, и отклонённые; ответ же обещал новую ступень всегда,
     * и регистратор читал одно, а карточка показывала другое. Хук уже
     * отработал к этому моменту, поэтому здесь просто читается результат.
     */
    const animalNow = await payload.findByID({
      collection: 'animals',
      id: animal.id,
      depth: 0,
      overrideAccess: true,
    })
    const raised = Number((animalNow as { trustLevel?: unknown })?.trustLevel ?? 0) === 2

    return {
      message:
        `Протокол № ${number} зарегистрирован.` +
        (raised
          ? ' Уровень достоверности записи — «Подтверждено лабораторией»;' +
            ' он снимется сам, если протокол отозвать.'
          : ' Уровень достоверности не изменился: он уже выше или снят решением Ассоциации.'),
      issuedId: created.id,
    }
  } catch (e) {
