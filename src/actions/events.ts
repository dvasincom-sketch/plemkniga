      ...
      await payload.update({
        ...
        data: {
          state: (['sold', 'culled', 'dead'].includes(state) ? state : 'sold') as
            ...
            | 'dead',
          /*
           * Дата выбытия — дата самого события. Без неё запись выпадала
           * из всех отчётов о выбытии: они считают по `disposalDate`,
           * а не по состоянию, и хозяйство, ведущее выбытие лентой,
           * не теряло коров ни по одному отчёту. Прежняя дата
           * не перетирается: первое выбытие честнее второго.
           */
          ...(ctx.animal.disposalDate ? {} : { disposalDate: date }),
