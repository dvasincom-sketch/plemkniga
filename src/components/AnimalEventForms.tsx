 ...
 * поля глазами. Пустое поле означает «не оценивали», а не ноль: и пятёрка
 * на шкале статей, и ноль на шкале композитов — середина, вполне осмысленная
 * оценка, а не пустота.
 *
 * Шкалы у двух половин бланка разные, и подпись это говорит. Прежде она
 * называла −2…+2 для всего бланка, хотя линейные статьи давно переехали
 * на 1–9: бонитёр, поверивший подписи, ставил бы в них отклонения.
 */
function ExteriorForm({
  ...
  return (
    <form action={formAction} className="card">
      ...
      <p className="mb-5 max-w-[70ch] text-[15px] leading-relaxed text-ink-700">
        Линейные признаки — по шкале 1–9, где 5 это среднее по породе. Композиты —
        отклонение, обычно −2…+2 (принимается до ±3), где середина это ноль. Пустое поле
        означает, что признак не оценивали, а не то, что он в середине шкалы. Прежняя
        оценка сохранится в истории, действующей станет эта.
      </p>
