 ...
 *
 * Показываются отдельно от находок эксперта и до них: сначала машина
 * говорит, куда смотреть, потом человек решает, что из этого — замечание
 * хозяйству. Кнопка «в замечания» переносит находку в его список одним
 * нажатием, вместе с формулировкой: переписывать её руками значит терять
 * время и вносить разночтения.
 *
 * Пропущенное не исчезает: при следующем открытии страницы проверки
 * прогоняются заново. Забыть находку нельзя, можно только не согласиться
 * с ней — и это правильное разделение труда.
 */
export function AutoIssues({
  id,
  issues,
  readOnly,
}: {
  ...
  const [, addAction] = useActionState<ReviewState, FormData>(addFindingAction, {})
    ...
    return (
      <div className="card">
        ...
        <p className="text-[15px] text-ink-700">
          Несостыковок не найдено: даты, пол родителей, границы правдоподобия продуктивности
          и полнота происхождения проверены.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="card">
      ...
      <p className="mb-5 max-w-[70ch] text-[15px] leading-relaxed text-ink-700">
        Найдено запросом, а не глазами: {fix} существенных, {issues.length - fix} на усмотрение.
        Это не решение — это «посмотрите сюда». Согласны с замечанием — перенесите его
        в список для хозяйства; не согласны — просто пропустите, правило написано программой,
        а не зоотехником.
      </p>

      <ul className="divide-y divide-[#ededed]">
        {issues.map((i, n) => (
          <li key={`${i.code}-${i.animalId}-${n}`} className="flex items-start justify-between gap-4 py-3">
