 ...
 * в `<label>` — подпись поля и поле в одном элементе. Пункт списка это
 * `<div role="option">`, для браузера не интерактивный элемент, поэтому
 * клик по нему считается кликом по подписи, и браузер переадресует его
 * связанному полю — нашей кнопке. Кнопка переключает состояние: только
 * что закрытый список открывается.
 *
 * По кнопке такого не бывает: клики по интерактивному потомку `<label>`
 * не переадресуются. Отсюда и странность — открывалось только после
 * выбора мышью и только внутри подписи.
 *
        ...
        >
          {all.map((o, i) => (
            <div
              ...
              onClick={(e) => {
                // Не даём внешнему `<label>` переадресовать клик кнопке — см. шапку
                e.stopPropagation()
                choose(i)
              }}
            >
