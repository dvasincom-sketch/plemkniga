 ...
 *
 * Надёжность стоит рядом со значением намеренно. Индекс 132 при сорока
 * трёх дочерях и 133 при тысяче двухстах — разные объекты, и показывать
 * их одинаково означало бы соврать. У чехов надёжность выписана
 * отдельными колонками по каждой части оценки; у нас она пока одна,
 * и это честнее, чем изобразить больше, чем посчитано.
 *
 * Колонка подписана «R, %», как в карточке животного, а не «Дост.»:
 * достоверностью в книге зовётся другое — уровень, до которого запись
 * подтверждена, — и в рейтинге, где обе величины у одного животного,
 * сокращение читалось не тем.
 *
  ...
  return (
    <div className="card mt-6">
        ...
        <table className="metric-table">
          <thead>
            <tr>
              ...
              <th className="text-right" title="Надёжность оценки, R %">
                R, %
              </th>
      ...
      <p className="mt-3 max-w-[90ch] text-[13px] leading-relaxed text-ink-500">
        ...
        что оценки по нему нет вовсе, — такое животное всё равно участвует в рейтинге, а его
        индекс собран из меньшего числа признаков. Сколько именно учтено, видно в карточке:
        надёжность рядом считается только по учтённым признакам и о пропущенных не говорит —
        животное с одной оценкой и животное со всеми одиннадцатью могут стоять здесь с одной
        и той же надёжностью.
      </p>
    </div>
  )
}
