    ...
    <section className="mt-9">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        {repro && (repro.calvings > 0 || repro.inseminations > 0) && (
          <Report
            ...
            why={
              <>
                ...
                {/*
                   Определение здесь стояло неверное — «до первого осеменения», —
                   и ровно так же считал запрос, тогда как ориентир 85–110 рядом
                   относится к плодотворному. Выгрузка во ФГИАС ПР при этом
                   всё время считала правильно, и одна книга отдавала в реестр
                   одно число, а зоотехнику показывала другое, оба называя
                   сервис-периодом. Разницу видно только на корове, забеременевшей
                   не с первого раза, — то есть на половине стада.
                */}
                <p>
                  Сервис-период — от отёла до плодотворного осеменения, ориентир 85–110 дней.
                  Дней до первого осеменения — про выявление охоты, а не про оплодотворяемость:
                  ориентир 60–80, и разница между двумя числами есть цена неудачных попыток.
          ...
          >
            {/*
               Плиток стало четыре, а не три: рядом с сервис-периодом встали дни
               до первого осеменения. Порознь каждое число мало что говорит,
               а вдвоём они разделяют две разные беды — не приходит в охоту
               и не оплодотворяется, — и лечатся они по-разному.
            */}
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              />
              <Tile
                label="Дней до первого осеменения"
                value={
                  repro.daysToFirstService === null ? '—' : nf(repro.daysToFirstService, 0)
                }
                note="ориентир 60–80"
              />
              <Tile
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {/*
                 ...
                 Список — свой, «culled-first». Прежде плашка вела на общий
                 список выбывших за год, и довод был такой: отдельный список
                 разошёлся бы с числом рядом. Он перестал быть верным в день,
                 когда список первотёлок завели: теперь число и список
                 считаются одним и тем же условием, а вело оно по-прежнему
                 на другое.
              */}
              <Tile
