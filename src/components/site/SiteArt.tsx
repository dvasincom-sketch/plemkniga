...
export function LayersArt({ labels, title }: { labels: string[]; title: string }) {
  ...
  return (
    /*
       Пирамида лежит на белой плашке — такой же, как карточки справа.

       ## Почему плашка

       В ряду стояли четыре предмета, и три из них были на заливке,
       а четвёртый — прямо на сером поле страницы. Читалось это как
       незаконченность: карточки выглядели предметом, схема — черновиком
       рядом с ним.

       Есть и вторая причина, менее очевидная и более важная. Нижняя
       полоса светло-серая (`ink-100`, #e5e7eb), а поле страницы —
       `canvas`, #f2f2f2. Разница между ними меньше, чем между соседними
       ступенями самой пирамиды: самая широкая полоса, то есть та, что
       несёт главное утверждение схемы («это есть у всех»), почти
       растворялась в фоне. На белом видны все три.

       ## Почему полосы от этого уменьшаются

       Плашка съедает свои поля, и пирамида внутри становится ниже
       на их величину. Это не потеря: высота здесь ничего не значит —
       смысл несёт ширина, то есть охват. Уменьшить можно то, что
       не сообщает ничего.
    */
    <div
      role="img"
