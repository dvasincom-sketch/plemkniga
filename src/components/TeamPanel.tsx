          ...
          {members.map((m) => (
            <li key={m.id} className="py-5 first:pt-0 last:pb-0">
              <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
                ...
                {/*
                   Должность подписана словом «должность», и это не
                   многословие.

                   Она стоит свободным текстом из трудовой — «зоотехник-
                   селекционер», «главный зоотехник», — а строкой ниже стоит
                   роль в хозяйстве, и одно из её трёх значений называется
                   «Зоотехник». Без подписи выходило так: «Зоотехник-
                   селекционер», а под ним «Руководитель · Ведёт данные,
                   приглашает сотрудников» — два разных ответа на один
                   и тот же с виду вопрос, вплотную друг к другу, на
                   единственной странице, где эта граница объясняется.

                   Развести их переименованием роли нельзя: слово «зоотехник»
                   выбрано в решении №107 как раз потому, что оно из этой
                   предметной области. Значит разводить надо не значения,
                   а вопросы, на которые они отвечают: «что написано
                   в трудовой» и «что человеку можно».
                */}
                {m.position && (
                  <span className="text-[14px] text-ink-500">должность: {m.position}</span>
                )}
              </div>

              <p className="mt-1 text-[13px] text-ink-500">
                права: {orgRoleLabel(m.orgRole)}
                {' · '}
