  ...
  return (
    <div className="block text-[14px]">
      ...
      <div className="mt-1.5 min-h-[18px] text-[13px] leading-snug">
        ...
        {/*
           Несостоявшийся поиск отличается от «не найдено».

           Строка ниже приглашает записать номер текстом и обещает, что
           связь установится позже; при сбое выборки это обещание
           необеспечено — животное может быть в книге, а мы его не увидели.
        */}
        {!checking && state?.failed && (
          <span className="text-ink-700">
            Поиск не состоялся — книгу сейчас не спросить. Номер записать можно,
            но проверить, есть ли такое животное, не удалось; повторите позже.
          </span>
        )}

        {!checking && state && !state.found && !state.failed && (
