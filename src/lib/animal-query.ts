...
export function buildAnimalWhere(
   ...
   *
   * Лежат в нём двоякие записи. Во-первых, служебные предки: заведены ради
   * построения родословных, в стаде никогда не стояли и в книге только
   * ломали счётчики. Во-вторых — с появлением срока хранения — записи,
   * которые хозяйство убрало само: ошибочно загруженные, не те, чужие.
   * И тем и другим место в архиве, а не в книге.
   *
   * Условие через `or`: у записей, заведённых до появления признака,
   * в поле NULL, а `not_equals: true` отбросил бы и их тоже.
   *
   * `opts.archive` переворачивает условие — показать ровно архив. Это
   * параметр вызова, а не адреса: будь он адресом, `?archive=1` открывал
   * бы архив любого хозяйства прямо в публичной книге. Спрашивать архив
   * вправе только страница, которая знает, чей он.
   */
  const and: Where[] = [opts?.archive ? { archived: { equals: true } } : NOT_ARCHIVED]
  ...
  /*
   * Кровность «не ниже» — для отбора чистопородных.
   *
   * Порог задаётся адресом, а не зашит: чистопородным в племучёте
   * считают 93,75 % (пятнадцать шестнадцатых), но требование это
   * не единственное в мире, и вписывать его в разбор значило бы
   * запретить любой другой вопрос к кровности.
   */
  const bloodFrom = num(sp.blood)
  if (bloodFrom !== undefined) and.push({ bloodPercent: { greater_than_equal: bloodFrom } })

  /*
   * Происхождение, подтверждённое ДНК.
   *
   * Спрашивается именно вывод теста, а не наличие записи о нём. Разница
   * та же, из-за которой в карточке появился `verdict`: тест, который
   * отцовство **исключил**, — это тоже проведённый тест, и отбор
   * «есть ДНК-тест» вернул бы записи с опровергнутым происхождением
   * в списке подтверждённых.
   */
  if (one(sp.dna) === '1') and.push({ 'dnaTests.verdict': { equals: 'confirmed' } } as Where)
...
export const FILTER_KEYS = [
  ...
  'forSale',
  /*
   * Кровность и ДНК стояли в `buildAnimalWhere` и в плашках быстрого
   * отбора, но не здесь — а этот список отвечает за то, какие условия
   * видны человеку. Отбор «Чистопородные» работал, панель условий о нём
   * молчала, и снять его можно было только «сбросить всё». Пропуск
   * дорогой ровно потому, что тихий: выдача сужена, объяснения нет.
   */
  'blood',
  'dna',
] as const
...
export const PRESETS = [
  ...
  /*
   * Подтверждённое ДНК происхождение — самый сильный признак качества
   * записи из всех, что книга умеет показать. Знак Ассоциации говорит
   * «документы сошлись», ДНК — «родители те самые».
   */
  { key: 'dna', label: 'Происхождение по ДНК', params: { dna: '1' } },
  /*
   * Чистопородные: 93,75 % и выше — пятнадцать шестнадцатых, обычная
   * граница чистопородности в племенном учёте.
   */
  { key: 'purebred', label: 'Чистопородные', params: { blood: '93.75' } },
  {
    key: 'sale',
