...
export const ICAR_SECTIONS: IcarSection[] = [
  ...
  },
  {
    ...
    ours:
      ...
      'экстерьера, взвешивания, племенная ценность. Приём POST по четырём из них — ' +
      'контрольные доения, отёлы, осеменения, взвешивания — с распознаванием ' +
      'повторной отправки по паре «источник + его номер записи», построчным ответом ' +
      'icarBatchResult и отказами в виде icarErrorResource.',
    gaps: [
      {
        what: 'Животные, оценка экстерьера и племенная ценность на запись закрыты.',
        why:
          'Это не пробел, а правило, и оно записано здесь, чтобы его не приняли ' +
          'за недоделку. Запись животного в книгу — утверждение происхождения, ' +
          'за которое Ассоциация отвечает перед заводчиком, и оно идёт заявкой ' +
          'с проверкой. Оценка экстерьера без аккредитованного оценщика — не оценка. ' +
          'Племенная ценность считается книгой: это результат, а не наблюдение.',
        need:
          'Ничего. Каждой из трёх коллекций при попытке записи отвечается 405 ' +
          'с объяснением словами, а не кодом: адрес существует и отдача по нему ' +
          'работает, отказано именно в записи.',
      },
      {
        what: 'Не все события покрыты.',
        why:
          'Отдаются семь коллекций из полутора десятков доступных. Нет движения ' +
          '(приход, выбытие, падёж), нет проверок стельности, нет запусков, нет ' +
          'наборов животных. Потребитель получает картину продуктивности без картины ' +
          'перемещений — а без неё непонятно, почему у коровы оборвался ряд доений.',
        need:
          'Отображение движения (icarMovementArrival/Departure/DeathEventResource), ' +
          'проверок стельности и запусков. Работа механическая, отображения уже написаны ' +
          'по одному образцу.',
      },
      {
        what: 'Соответствие схемам не проверяется автоматически.',
        why:
          'Наш прогон сверяет ответ с нашей же копией перечислений. Если ICAR изменит ' +
          'перечисление, мы узнаем об этом от партнёра, а не от себя.',
        need:
          'Сверка ответа с настоящими JSON-схемами из репозитория adewg/ICAR — либо ' +
          'загрузкой схем в прогон, либо их копией, обновляемой руками с отметкой даты.',
      },
    ],
  },
  {
