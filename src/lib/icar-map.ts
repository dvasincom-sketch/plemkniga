...
export const ICAR_SECTIONS: IcarSection[] = [
  ...
  },
  {
    ...
    ours:
      ...
      'экстерьера, взвешивания, племенная ценность.',
    gaps: [
      {
        what: 'Приём данных по ADE не открыт: только отдача.',
        why:
          'Запись, пришедшая снаружи, должна пройти те же полсотни проверок, что ' +
          'и введённая руками, иначе обмен станет дырой, через которую в книгу ' +
          'попадёт то, чего в неё не пускают через форму. Открыть POST раньше — ' +
          'обменять достоверность книги на галочку в списке возможностей.',
        need:
          'Приём POST с прогоном через тот же разбор и те же проверки, что у загрузки ' +
          'файлов, плюс ответ icarBatchResult с построчными ошибками.',
      },
      {
