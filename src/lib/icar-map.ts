...
export const ICAR_SECTIONS: IcarSection[] = [
  ...
  },
  {
    ...
    ours:
      ...
      'из репозитория adewg/ICAR, а не с нашей копией перечислений. ' +
    ...
    gaps: [
      {
        what: 'Пакетные адреса /batches/... не заведены.',
        why:
          'Стандарт разводит одиночную запись и пакет по разным адресам. Мы ' +
          'принимаем массив на том же адресе: разводить их значило бы завести ' +
          'вторую копию проверки прав и опознания локации — второе место, где ' +
          'право написать в чужое хозяйство может разойтись с первым. ' +
          'Отклонение осознанное, но это всё же отклонение.',
        need:
          'Либо отдельные адреса /batches/..., переиспользующие ту же проверку ' +
          'прав, либо согласование с ADEWG, что массив на основном адресе ' +
          'считается допустимым прочтением.',
      },
      {
        what: 'Generic Data Exchange API не реализован.',
        why:
          'Вторая половина раздела — обмен через датасеты и ленту изменений: ' +
          'четыре адреса, клиенту не нужно знать про локации, синхронизация идёт ' +
          'по непрозрачному токену. Это то, чем пользуются при постоянной ' +
          'двусторонней связи между системами; у нас работает только выборка ' +
          'по локациям, то есть партнёру приходится обходить хозяйства самому.',
        need:
          'Четыре адреса: /datasets, /datasets/{id}, /datasets/{id}/changes?since=, ' +
          '/dataset/{id}/resources. Плюс лента изменений с токеном продолжения ' +
          'и заголовком icar-full-sync при пересинхронизации.',
      },
      {
        what: 'Фильтры выборки поддержаны частично.',
        why:
          'Стандарт задаёт имена фильтров и правила их составления: поле называется ' +
          'как само поле, вложенное — через дефис, диапазон — суффиксами -from и -to. ' +
          'У нас есть только отбор по дате изменения. Клиент вправе прислать ' +
          'animal-id или birthDate-from и получить больше данных, чем просил, — ' +
          'это стандартом допускается, но каждый лишний мегабайт платит партнёр.',
        need:
          'Разбор рекомендованных фильтров: animal-id + animal-scheme, ' +
          'meta-created-from/-to, meta-source. Имена — строго из стандарта: ' +
          'свои имена хуже отсутствующих фильтров.',
      },
    ],
  },
  {
