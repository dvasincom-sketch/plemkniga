 ...
 * случившегося: база недоступна, в журнале осталась отметка `dev` —
 * и приложение при старте спрашивает разрешения на миграции у контейнера,
 * в котором некому отвечать.
 *
 * Сверки журнала со схемой здесь нет, хотя шапка её обещала: она стоит
 * десятков запросов к базе (по опорному объекту на каждую миграцию),
 * а осмотр должен отвечать за секунды и гоняется ночной пробой.
 * Живёт сверка в `npm run migrate:baseline`.
 *
...
export async function runDoctor(): Promise<DoctorCheck[]> {
  ...
  try {
    ...
    const hasJournal = journalExists.rows[0]?.t !== null
    if (!hasJournal) {
      add({
        ok: false,
    ...
    /*
     * Шаг считает строки журнала — и только. Назывался он «схема против
     * журнала», и реестр обещал сверку схемы с журналом; сверки здесь нет
     * и не было, а шаг, который не бывает красным, читается как «сошлось».
     * Сверку делает `migrate:baseline`: она ищет опорный объект каждой
     * миграции в самой базе, а это десятки запросов — в осмотре, который
     * должен отвечать за секунды, им не место.
     *
     * Отбор по `name <> 'dev'` заменён на `batch <> -1`: шагом выше
     * объяснено, почему отметку разработки Payload узнаёт по номеру пакета,
     * а не по имени, — и два шага одного осмотра не должны спорить
     * о том, что считать отметкой.
     *
     * Запрос выполняется только при живом журнале: без него он бросает,
     * и вместе с ним пропадала бы находка «журнала нет» вместе со своим
     * советом — то есть осмотр молчал бы ровно в самом плохом случае.
     */
    if (hasJournal) {
      const applied = await pool.query<{ n: string }>(
        `select count(*)::text as n from payload_migrations where batch <> -1 or batch is null`,
      )
      add({
        ok: true,
        title: `Миграций записано в журнал: ${ru(Number(applied.rows[0]?.n ?? 0))}`,
        detail: 'Это счёт строк, а не сверка со схемой. Сверка — npm run migrate:baseline',
      })
    }
