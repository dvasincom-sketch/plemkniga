...
/**
 * Тест на происхождение — по выводу, а не по наличию записи.
 *
 * Раньше здесь стояло `filled(animal.dnaTests)`: требование выполнялось
 * **наличием любой записи** о ДНК-тесте. Тест, который отцовство исключил,
 * его удовлетворял, и свидетельство выпускалось. Проверка, молча
 * пропускающая ровно то, ради чего она есть, опаснее отсутствующей —
 * на неё полагаются.
 *
 * Теперь нужен вердикт «подтверждено». У тестов, заведённых до появления
 * поля, вердикта нет, и требование для них не выполнено — это верно
 * по существу: система не знает, что показал тот тест, и делать вид,
 * что знает, нельзя.
 */
const parentageRequirement = (animal: Animal): Requirement => {
  const tests = animal.dnaTests ?? []
  const confirmed = tests.some((t) => t.verdict === 'confirmed')
  const excluded = tests.some((t) => t.verdict === 'excluded')

  return {
    key: 'dna',
    label: 'Происхождение подтверждено ДНК-тестом',
    ok: confirmed && !excluded,
    fix: excluded
      ? 'ДНК-тест исключил происхождение. Свидетельство по такой записи не выпускается: сначала надо исправить родословную'
      : filled(tests)
        ? 'У теста не проставлен вывод. Откройте вкладку «Генетика» и укажите, подтвердил он происхождение или нет'
        : 'Добавьте результат ДНК-теста во вкладке «Генетика». В сертификате это раздел 8 — метод и результат по каждому родителю',
  }
}
