 ...
 *
 * Вопрос не праздный. Payload включает push всякий раз, когда `NODE_ENV`
 * не равен `production`, и смотрит только на это. Строка подключения его
 * не интересует. А значит любая команда, запущенная с машины разработчика
 * против боевой базы — выгрузка, пересчёт, ревизия, — молча приведёт боевую
 * схему к тому, что лежит в рабочей копии, и оставит в журнале миграций
 * отметку `dev`.
 *
 * Так однажды и вышло: прод перестал подниматься, потому что при старте
 * увидел эту отметку и стал спрашивать разрешения на миграции у контейнера,
 * в котором некому отвечать.
 *
 * Признак локальности грубый — петлевой адрес или сокет. Ошибиться он может
 * только в одну сторону: посчитать локальную базу удалённой и не сделать
 * push. Это неудобно, но безопасно, а обратная ошибка стоит прода.
 */
export function isLocalDatabase(uri?: string | null): boolean {
  if (!uri) return false
  // Подключение через unix-сокет адреса не содержит вовсе — оно всегда местное
  if (uri.startsWith('/') || uri.includes('host=/')) return true
...
/**
 * Приводить ли схему базы к конфигу на лету.
 *
 * ## Почему решение вынесено сюда
 *
 * Считалось оно в `payload.config.ts`, а `/healthz` печатал вместо него
 * переменную окружения: `PAYLOAD_DB_PUSH ?? 'true (по умолчанию)'`.
 * Это разные вещи. Переменная — половина условия; вторая половина
 * в том, местная ли база. На боевом сервере push выключен как раз
 * второй половиной, а страница состояния уверенно писала «true».
 *
 * Цена такой уверенности не теоретическая: по этой строке был сделан
 * вывод, что прод приводит схему сам и потому не прогоняет миграции.
 * Вывод неверный — прод их прогоняет (`prodMigrations`), и в той же
 * выдаче стояло «applied 51». Страница состояния, отвечающая
 * на вопрос, которого ей не задавали, хуже её отсутствия: ей верят.
 *
 * Теперь решение одно на двоих: конфиг им пользуется, `/healthz`
 * его же и показывает.
 */
export const shouldPushSchema = (driverUri?: string | null): boolean =>
  process.env.PAYLOAD_DB_PUSH !== 'false' && isLocalDatabase(driverUri)
