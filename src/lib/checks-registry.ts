...
/**
 * Группа правил по-русски.
 *
 * Английского названия здесь больше нет: он уехал в словарь языка вместе
 * с остальными пятью (`i18n/data/checks.en.ts`). Пара `label` / `labelEn`
 * держалась на том, что языков два; на шести она превращала запись
 * в двенадцать строк подписей и заставляла страницу выбирать язык руками.
 */
...
export type CheckSpec = {
  ...
  why: string
  /**
   * Числовая граница — у тех правил, у которых она есть.
   *
   * Раньше тип требовал границу парой («либо нет вовсе, либо на обоих
   * языках»): русское «2,0…6,5 %» на английской странице читается как два
   * числа подряд, и пропущенный `thresholdEn` был бы виден только читателю.
   * Пары больше нет, но требование осталось — оно переехало в тип словаря
   * (`CheckTexts` ниже) и стало сильнее: граница обязана быть на всех пяти
   * языках, а не на одном английском.
   */
  threshold?: string
...
export const PLAUSIBLE = {
  ...
  ageYears: 25,
  /**
   * Межотельный период, дни.
   *
   * Границы стояли в двух отчётах разными числами: 300…600 в сводке
   * по стаду и 250…900 в отчёте о возрасте первого отёла. Одна и та же
   * корова попадала в среднее одного отчёта и не попадала в среднее
   * другого, а объяснить разницу было нечем — оба числа подписаны
   * «межотельный период».
   *
   * Взяты широкие: узкие отсекают ровно тех коров, ради которых
   * показатель и смотрят, — с затянувшимся периодом. Нижняя граница
   * чуть больше стельности: короче физически не бывает, это описка
   * в дате или незаписанный аборт.
   */
  calvingInterval: { min: 250, max: 900, unit: 'дней' },
  /**
   * Дни от отёла до первого осеменения.
   *
   * Раньше двадцатого дня матка не восстановилась — такое осеменение
   * описка в дате; после двухсот пятидесяти это уже не выявление охоты,
   * а корова, о которой забыли на восемь месяцев.
   */
  firstService: { min: 20, max: 250, unit: 'дней' },
} as const
...
export const CHECKS = [
  ...
  },
  {
    ...
    why: 'То же самое, что с жиром, и опаснее: белок весит в профиле Ассоциации четырнадцать процентов, а разброс по нему в полтора раза уже, чем по жиру (σ 6,93 против 11,29 кг). Лишние сорок килограммов белка вытаскивают животное в первый процент книги.',
    severity: 'fix',
  ...
  {
    /*
     * Правило заведено последним из трёх и оказалось самым важным.
     *
     * Соседние два смотрят на состояние и на причину, и оба вместе
     * не замечали пропажи третьего поля — даты. Между тем по ней, а не
     * по состоянию, считаются все отчёты о выбытии: животное с состоянием
     * «выбраковано» и пустой датой уходило из знаменателя и не попадало
     * в числитель, то есть занижало долю выбытия дважды. Ошибки при этом
     * не возникало нигде, и заметить её было нечем — ровно тот класс
     * находок, ради которого проверки и заведены.
     *
     * Уровень «починить», а не «замечание»: отсутствующая причина портит
     * раскладку, отсутствующая дата убирает животное из отчёта целиком.
     */
    code: 'disposal-date-missing',
    group: 'lifecycle',
    label: 'Выбытие без даты выбытия',
    what: 'Состояние не «в стаде», а дата выбытия не проставлена.',
    why: 'Отчёты о выбытии считают по дате, а не по состоянию: без неё животное не попадёт ни в один из них, а доля выбытия по стаду выйдет ниже настоящей.',
    severity: 'fix',
  },
...
/* -------------------------------------------------------------------------
 * Слова правил на других языках
 *
 * Сами слова лежат не здесь, а в словарях `i18n/data/checks.<язык>.ts` —
 * по файлу на язык. Разбор решения написан в `i18n/data-text.ts`; коротко:
 * русский остаётся в записи, потому что он источник и стоит рядом
 * с порогами, а словарь на языке отдают носителю целиком и целиком же
 * принимают обратно. Здесь остаются только типы, потому что полнота
 * словаря проверяется по реестру, а реестр — здесь.
 * ---------------------------------------------------------------------- */

/** Слова одного правила на одном языке. */
export type CheckText = {
  /** Подпись правила — заголовок его карточки. */
  label: string
  /** Что именно сверяется. */
  what: string
  /** Почему это стоит ловить. */
  why: string
  /** Числовая граница — там, где она есть по-русски. */
  threshold?: string
}

/**
 * Правила, у которых числовая граница есть по-русски.
 *
 * Выведены из самого реестра, а не перечислены руками: список, написанный
 * рядом, разошёлся бы с реестром на первом же новом правиле с порогом —
 * и разошёлся бы молча, потому что проверять его нечем.
 */
export type ThresholdCheckCode = Extract<(typeof CHECKS)[number], { threshold: string }>['code']

/**
 * Словарь правил на одном языке.
 *
 * Полон по обоим измерениям: правило, заведённое без перевода,
 * не соберётся, и граница, потерянная при переводе, не соберётся тоже.
 * Второе важнее, чем кажется: пропущенная граница не выглядит ошибкой —
 * карточка просто теряет строку с числом, и заметить это может только
 * тот, кто помнит русскую.
 *
 * Обратное тоже запрещено: граница у правила, у которого её по-русски
 * нет, означала бы число, придуманное переводчиком.
 */
export type CheckTexts = {
  [C in CheckCode]: CheckText &
    (C extends ThresholdCheckCode ? { threshold: string } : { threshold?: undefined })
}

/** Слова одной группы правил на одном языке. */
export type CheckGroupText = {
  label: string
  intro: string
}

export type CheckGroupTexts = Record<CheckGroup, CheckGroupText>
 ...
 *
 * Код при этом остаётся узким. В `CheckSpec` он объявлен строкой, иначе
 * вышел бы круг: `CheckCode` выведен из реестра, а реестр обязан
 * удовлетворять `CheckSpec`. Но словари языков лежат под `CheckCode`,
 * и перебор, отдающий строку, доставал бы из них слова приведением —
 * то есть обещанием системе типов того, чего она не проверила.
 */
export type CheckEntry = CheckSpec & { code: CheckCode }

export const ALL_CHECKS: readonly CheckEntry[] = CHECKS
