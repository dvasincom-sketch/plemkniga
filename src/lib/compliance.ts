 ...
 *
 * ## Почему переводов позиций здесь нет
 *
 ...
 * абзац посреди чужой страницы отвечает на этот вопрос раньше и хуже
 * любого текста.
 *
 * Ответом были парные поля — `title` и рядом `titleEn`. На двух языках
 * это работало, на шести перестало. Пять переводов у каждой из четырёх
 * строк прозы утопили бы русские формулировки, ради которых в этот файл
 * и заходят; хуже того, страница выбирала язык руками, а признак
 * «страница английская» ложен на казахской — и тело оставалось русским
 * под переведённой рамкой, ни разу об этом не сказав.
 *
 * Поэтому русский остался здесь, где стоят и сами оценки, а каждый
 * другой язык лежит отдельным словарём `i18n/data/compliance.<язык>.ts`
 * и полон по типу: позиция, добавленная без перевода, не соберётся —
 * и это верно про все пять языков, а не про один английский. Разбор
 * решения целиком — в шапке `i18n/data-text.ts`.
 *
 * ## Где смотреть
 *
...
/**
 * Подписи состояний по-русски; пять остальных языков — в `i18n/data`.
 *
 * `Record` по состоянию, а не список: новое состояние не соберётся без
 * подписи, и в сводке не окажется пустой плашки рядом с числом.
 */
 ...
 * Ключи позиций.
 *
 * Список ключей отдельным типом — не педантизм, а то, чем держится
 * перевод. Словарь языка объявлен `Record<ComplianceKey, …>`, и позиция,
 * добавленная сюда без строки в пяти словарях, сборку роняет. Пока ключ
 * был просто строкой, забыть перевод можно было молча — и забывали.
 */
...
/**
 * Переводимая часть позиции — ровно то, что лежит в словарях языков.
 *
 * Всё остальное — состояние, область, доказательства, адрес источника —
 * от языка не зависит и потому не повторяется в пяти файлах: число,
 * переписанное шесть раз, шесть раз и расходится.
 */
export type ComplianceText = {
   ...
   * Что дальше.
   *
   * У выполненного и у чужой области следующего шага нет, поэтому поле
   * необязательное. Там же, где оно есть, оно и есть самое ценное
   * на странице: именно в нём сказано, чего не хватает.
   */
  next?: string
  /**
   ...
   */
  external?: string
  /** Подпись ссылки на источник: адрес один на все языки, подпись — нет. */
  source?: string
}

/**
 * Позиция реестра: русский текст и всё, что от языка не зависит.
 *
 * Поля прозы повторяют `ComplianceText` по именам, но тип не собирается
 * пересечением: у источника здесь пара «подпись и адрес», а в словаре
 * языка — одна подпись. Адрес переводить нечего, и заводить его пять раз
 * значило бы завести пять мест, где он может разойтись.
 */
export type ComplianceItem = Omit<ComplianceText, 'source'> & {
  key: ComplianceKey
  area: ComplianceArea
...
export const COMPLIANCE: ComplianceItem[] = [
  ...
  },
  {
    key: 'ade',
    /*
     * В заголовке минорная линия, а не точная версия из `ADE_VERSION`, и это
     * не расхождение. Поддержку стандарта заявляют именно линией: в файлах
     * самого ADE стоит `info.version: "1.5"`, а операции помечены тегами вида
     * `ADE-1.5-milk`. Точный релиз, по которому сверены схемы, живёт константой
     * в `ade/core.ts` и меняется с каждым патчем — в заголовке позиции он
     * означал бы, что с выходом 1.5.2 книга перестала соответствовать.
     */
  ...
  /* ------------------------ Племенное дело ------------------------- */
  {
     ...
     *
     * Здесь стояло «пятнадцать», и оно разошлось с разбором: разбор
  ...
  },
  {
    key: 'iso27001',
    external:
      'аккредитованный аудитор: сертификацию выдаёт орган по сертификации, а не разработчик',
  ...
  },
  {
    key: 'gost-rbpo',
    external:
      'орган по оценке соответствия: практики можно выполнять, соответствие — только ' +
      'подтвердить извне',
 ...
 * очевидны, но и среди «в плане» есть такие, которые тоже своими силами
 ...
 * значило бы обещать работу, которой нет. Сколько их сейчас — вопрос
 * к самому реестру, а не к этой оговорке: числа здесь нет намеренно,
 * потому что реестр пополняется, а комментарий рядом — нет.
 */
