...
export const COMPLIANCE: ComplianceItem[] = [
  ...
  },
  {
    ...
    ours:
      'Семь коллекций на отдачу по адресам спецификации: животные, контрольные доения, ' +
      'отёлы, осеменения, оценки экстерьера, взвешивания, племенная ценность. Схемы ' +
      'идентификаторов от домена Ассоциации.',
    evidence: [
      { kind: 'check', value: 'check:ade' },
      { kind: 'code', value: 'src/lib/ade/core.ts' },
      { kind: 'doc', value: 'docs/ade-spec.md' },
    ],
    next:
      'Приём POST закрыт сознательно: запись снаружи должна пройти те же проверки, ' +
      'что и ручной ввод. Нет движения, проверок стельности и запусков. Сверка идёт ' +
      'с нашей копией перечислений, а не с настоящими схемами репозитория.',
  ...
  },
  {
    ...
    ours: 'OpenAPI-описание, страница документации и прогон, сверяющий описание с кодом.',
    evidence: [
  ...
  /* ------------------------ Племенное дело ------------------------- */
  {
    ...
    next: 'Пробелов пятнадцать по девяти разделам — все выписаны на отдельной странице.',
