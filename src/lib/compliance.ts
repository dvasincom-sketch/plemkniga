...
export const COMPLIANCE: ComplianceItem[] = [
  ...
  },
  {
    ...
    next:
      'Пакетные адреса /batches/... не заведены — массив принимается на основном ' +
      'адресе, чтобы не заводить вторую копию проверки прав. Generic Data Exchange ' +
      'API с лентой изменений не реализован вовсе. Фильтры выборки поддержаны ' +
      'частично: есть только отбор по дате изменения. ' +
