...
export const NATIONAL_PROFILES: IndexProfile[] = [
  ...
  /*
   * SIH — чешский селекционный индекс голштинов, и стоит он здесь
   * не для полноты коллекции.
   *
   * Ассоциация взяла за образец работу чешских заводчиков, а два индекса
   * выше — американской школы. Разница между школами не в оттенках весов,
   * она в одном признаке: у NM$ экстерьер практически обнулён (корпус
   * идёт с минусом, вымя и ноги весят по баллу), у SIH экстерьер это
   * четверть индекса целиком. Держать оба рядом — единственный способ
   * увидеть, что «лучшая корова» по одному профилю и по другому это
   * разные коровы, и что выбор между ними делает человек, а не формула.
   *
   * ## Откуда веса
   *
   * Опубликованная формула: SIH = 0,49·DSI-МЛК + 0,25·DSI-ЗЕВ
   * + 0,12·DSI-ПЛД + 0,07·RPH-sb + RPH-длв. Коэффициент при долголетии
   * в источнике не выписан; на оставшиеся семь сотых он и приходится,
   * и так здесь и взято. Это чтение, а не цитата, — если найдётся точная
   * публикация, число поправить.
   *
   * ## Что здесь приближение, а что нет
   *
   * Пять слагаемых верхнего уровня взяты как опубликованы. А вот из чего
   * складывается сам DSI-МЛК и сам DSI-ЗЕВ, в том же виде не публикуется,
   * и разбивка 49 на молоко-жир-белок и 25 на три композита — наша,
   * по той же логике приближения, что у NM$ и TPI: белок тяжелее жира,
   * потому что чешская селекция идёт под сыр, вымя тяжелее ног и корпуса,
   * потому что так устроен любой экстерьерный индекс.
   *
   * Корпус здесь с плюсом, и это не описка. У NM$ он с минусом, потому
   * что крупная корова дороже в содержании и американский индекс считает
   * деньги. Чешский считает породу, и рост корпуса в нём — улучшение.
   * Ровно об этом расхождении и речь.
   *
   * Лёгкости отёла и смертности приплода в SIH нет: у чехов они вынесены
   * в отдельный индекс здоровья и в селекционный не входят. Здесь они
   * поэтому не проставлены вовсе, а не проставлены нулями.
   */
  {
    key: 'sih',
    name: 'SIH (приближение)',
    hint: 'Чехия, Holstein CZ: продуктивность и экстерьер вместе. Экстерьер — четверть индекса',
    kind: 'selection',
    owner: null,
    weights: {
      milk: 8,
...
export type IndexResult = {
   ...
   *
   * `null` — собрать её не из чего: ни у одного учтённого признака нет
   * привезённой надёжности. Это не то же, что ноль, и показывается
   * прочерком.
   */
  reliability: number | null
...
export function computeIndex(
  ...
  for (const [key, rawWeight] of Object.entries(weights)) {
    ...
    contributions.push({
      ...
      standardized,
      /*
       * Вес отдаётся с уже применённым направлением признака, иначе
       * строка разбора сама себе противоречит: у смертности приплода
       * стояло бы «прогноз +1,2, вес +490, вклад −588». Читатель,
       * перемножающий два первых числа, получал третье с другим знаком
       * и справедливо решал, что где-то ошибка.
       *
       * У селекционного профиля разворот уже сидит в стандартизованном
       * отклонении, и второй здесь вернул бы знак обратно.
       */
      weight: profile.kind === 'economic' ? weight * direction : weight,
      points,
      reliability: r,
    })
  }
...
function indexReliability(
   ...
   * Пусто, а не ноль.
   *
   * Ноль здесь означал бы «надёжность измерена и оказалась нулевой»,
   * то есть утверждение «оценке верить нельзя». Пустой знаменатель
   * означает другое: ни у одного учтённого признака надёжности
   * не прислали, и сказать о ней нечего. Рейтинг печатал «0 %» —
   * и это был приговор, вынесенный за отсутствие данных.
   *
   * Тип и подписи под таблицами переписали раньше самой функции:
   * `IndexResult.reliability` объявлен `number | null`, а страницы
   * научены показывать прочерк, — но функция по-прежнему возвращала
   * ноль, и обе ветки с прочерком были мёртвыми. Починка, ушедшая
   * не туда: числа на экране не сдвинулись, и это должно было
   * насторожить сразу.
   */
  if (!den) return null
