...
const SECTIONS = [
  ...
  },
  {
    ...
    /*
     * Надгробия удалённых ресурсов стоят здесь, а не в служебных, хотя
     * пишутся только хуками и читаются только Ассоциацией.
     *
     * Довод не в том, что ими пользуются, а в том, что они объясняют
     * ленту изменений. Партнёр, увидевший в ленте ресурс с `isDeleted`,
     * вправе узнать, откуда тот взялся и почему приходит спустя полгода
     * после удаления. Спрятать таблицу значило бы оставить единственное
     * объяснение в коде.
     */
    slugs: ['ade-tombstones'],
  },
  {
    key: 'Прочее',
    description:
...
export function buildOpenApi(payload: Payload, serverUrl: string): OpenApiDocument {
  ...
  /* ------------------------------------------------------------------ *
   *  Обмен наборами: вторая половина пятнадцатого раздела              *
   * ------------------------------------------------------------------ */

  paths['/ade/v1/datasets'] = {
    get: {
      tags: [adeTag],
      summary: 'Перечень наборов данных',
      description:
        'Вход в обмен наборами. Клиенту не нужно знать про хозяйства: он читает ' +
        'перечень, берёт из набора адрес ленты и ходит по нему. Адреса относительные — ' +
        'как в примерах стандарта.',
      responses: { '200': { description: 'Массив описаний наборов' } },
    },
  }

  paths['/ade/v1/datasets/{dataset}'] = {
    get: {
      tags: [adeTag],
      summary: 'Один набор данных',
      parameters: [
        {
          name: 'dataset',
          in: 'path',
      ...
      responses: {
        '200': { description: 'Описание набора' },
        '404': { description: 'icarErrorResource: неизвестный набор' },
      },
    },
  }

  paths['/ade/v1/datasets/{dataset}/changes'] = {
    get: {
      tags: [adeTag],
      summary: 'Лента изменений набора',
      description:
        'Массив: первым идёт @context, далее ресурсы в порядке правки, последним — ' +
        'метка продолжения @continuation. Удалённое приходит ресурсом с meta.isDeleted. ' +
        'Клиент хранит метку у себя и присылает её в since; сервер ничего о клиенте ' +
        'не помнит. Метка непрозрачна и подписана: изменённая отвергается, а метка ' +
        'прежней версии отвечается заголовком icar-full-sync: true — это указание ' +
        'выбросить местную копию набора и начать сначала.',
      parameters: [
        {
          name: 'dataset',
          in: 'path',
        ...
        },
        {
          ...
          required: false,
          schema: { type: 'string' },
          description: 'Метка продолжения из прошлого ответа. Без неё лента идёт с начала.',
        },
        { name: 'pageSize', in: 'query', required: false, schema: { type: 'integer' } },
      ],
      responses: {
        '200': { description: 'Контекст, ресурсы, метка продолжения' },
        '400': { description: 'icarErrorResource: негодная метка' },
        '404': { description: 'icarErrorResource: неизвестный набор' },
      },
    },
  }

  paths['/ade/v1/datasets/{dataset}/resources'] = {
    post: {
      tags: [adeTag],
      summary: 'Отправить ресурсы в набор',
      description:
        'Приём в набор. Тело — массив: @context, затем ресурсы; метки продолжения ' +
        'в посылке быть не должно. Хозяйство берётся из поля location каждого ресурса. ' +
        'Ответ 200 означает, что принято всё; одна негодная запись — четырёхсотый ' +
        'на всю посылку, потому что по стандарту любой иной код клиент понимает как ' +
        '«не сохранено ничего». Повторная отправка безопасна: приём опознаёт её ' +
        'по паре «источник и его номер записи». Удаление по обмену не принимается. ' +
        'Тот же обработчик отвечает по адресу /ade/v1/dataset/{dataset}/resources — ' +
        'в единственном числе, как написано в тексте спецификации.',
      parameters: [
        {
          name: 'dataset',
          in: 'path',
      ...
      responses: {
        '200': { description: 'Принято целиком' },
        '400': { description: 'icarErrorResource: посылка не принята ни в какой части' },
        '404': { description: 'icarErrorResource: неизвестный набор или чужая локация' },
        '405': { description: 'icarErrorResource: набор только на чтение' },
      },
    },
  }
  ...
  const orderedTags = [
    ...SECTIONS.flatMap((section) => [
      ...
      ...(section.key === 'Доступ'
        ...
        : []),
      /*
       * Тег обмена объявляется здесь руками, потому что за ним не стоит
       * коллекция Payload: `orderedTags` собирается из `slugs`, а адреса
       * стандарта живут своими маршрутами. Без этой строки тег встречался
       * бы у ручек, но не был бы объявлен — Swagger UI вывел бы его в конец,
       * ниже всех объявленных, и «Обмен» оказался бы не на своём месте.
       * Ловит это `check:openapi`.
       */
      ...(section.key === 'Обмен'
        ? [
            {
              ...
              description:
                'Стандарт ICAR ADE 1.5, оба способа обмена. Выборка по локациям: ' +
                'отдача одиннадцати коллекций, приём событий у уже записанных ' +
                'животных, пакетные адреса. Обмен наборами: перечень наборов, ' +
                'лента изменений с меткой продолжения, приём в набор.',
            },
          ]
        : []),
    ]),
