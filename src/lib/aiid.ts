 ...
 *
 * ## Третий случай, и он спорный
 *
 * Страна происхождения известна, а её номер — нет: животное ввезли,
 * записали откуда, но чужой номер не переписали. Тогда номер собирается
 * из **чужой страны и нашего национального номера** — `BLRM000000000777`
 * у коровы, ввезённой из Беларуси. Так закреплено прогоном
 * (`check:aiid`), и это осознанный выбор, а не недосмотр: без него
 * у половины импорта международного номера не было бы вовсе.
 *
 * Цена выбора названа здесь, чтобы её не пришлось открывать заново.
 * Такой номер выглядит как белорусский, а белорусским не является:
 * в реестре Беларуси под ним записано другое животное или никакое.
 * Пока номер живёт внутри книги, это неудобство; в тот день, когда его
 * начнут отдавать в международный обмен, это становится утверждением
 * о чужом реестре, которого мы не вправе делать.
 *
 * Правильный выход — не собирать номер вовсе, пока чужой не записан,
 * и потребовать его при вводе импортного животного. Решение за
 * Ассоциацией: оно меняет номера у части уже заведённых записей.
 */
