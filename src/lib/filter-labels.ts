...
export function describeFilter(
  ...
  switch (key) {
    ...
    /*
     * Кровность и ДНК описаны здесь потому, что отбирают по ним давно,
     * а показывать фишкой было нечем: обоих ключей не было в `FILTER_KEYS`.
     * Плашки «Чистопородные» и «Происхождение по ДНК» отбор задавали,
     * а панель условий о нём молчала — снять его можно было только
     * «сбросить всё», то есть вместе со всем остальным.
     */
    case 'blood':
      return { label: 'Кровность', value: `от ${value} %` }
    case 'dna':
      return { label: 'Происхождение', value: 'подтверждено ДНК' }
    case 'ipcFrom':
