...
/**
 * Служебная часть ресурса — и главное в ней: `sourceId`.
 *
 * ## Почему номер записи всегда с приставкой раздела
 *
 * Стандарт требует, чтобы `sourceId` был уникален **среди всех ресурсов
 * всех наборов одного сервера**, а не внутри своей коллекции. У нас же
 * номера начинаются с единицы в каждой таблице: контрольное доение №5
 * и отёл №5 отдавались обоих как «5».
 *
 * Пока обмен шёл по локациям, это сходило с рук: раздел был виден
 * из адреса запроса. В ленте изменений адрес один на все ресурсы,
 * и клиент раскладывает их по `sourceId` — два разных события легли бы
 * в одну запись, и последнее затёрло бы первое. Тихо: ни ошибки,
 * ни расхождения счётчиков, просто у коровы пропадает отёл.
 *
 * Поэтому приставка — имя раздела стандарта, и она не имеет исключений.
 * У животного `sourceId` и без неё уникален (это uuid), но правило
 * с одним исключением люди применяют неверно чаще, чем правило без них.
 */
export const adeMeta = (opts: {
  scope: AdeCollectionName
  ...
  ...(opts.sourceId !== null && opts.sourceId !== undefined
    ? { sourceId: `${opts.scope}:${opts.sourceId}` }
    : {}),
  /*
