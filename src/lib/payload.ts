 ...
 */
/**
 * Один ответ на запрос, а не столько, сколько раз спросили.
 *
 * ## Что было
 *
 * «Кто вошёл» спрашивают дважды на каждой странице: шапка сайта — чтобы
 * показать имя и счётчик уведомлений, и сама страница — чтобы решить,
 * пускать ли. Спрашивают независимо и не знают друг о друге, и это
 * правильно: шапка не должна зависеть от страницы, а страница от шапки.
 *
 * Неправильно было то, что каждый вопрос стоил разбора токена и запроса
 * к таблице пользователей. Дважды на каждое открытие любой страницы —
 * не катастрофа, но это половина всех запросов на лёгких экранах,
 * и растёт она вместе с числом мест, где спрашивают.
 *
 * ## Почему `cache` из React, а не своя переменная
 *
 * Память здесь нужна ровно на один запрос и ни секундой дольше: между
 * запросами пользователь другой. Модульная переменная жила бы дольше
 * запроса и отдала бы одного человека другому — ошибка, которая
 * не проявляется на своей машине и проявляется на первом же
 * одновременном посетителе.
 *
 * `cache` живёт в границах одного рендера и снимается сам. Вне рендера —
 * в скрипте, в замере — он просто не запоминает ничего, и это верное
 * поведение: там запроса нет вовсе.
 */
