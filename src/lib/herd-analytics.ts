...
export async function reproduction(
  ...
  const res = await pool.query(
     ...
     *
     * Рамка правдоподобия ниже часть таких промежутков гасила, и потому
     * ошибка не бросалась в глаза: число оставалось правдоподобным
     * и просто занижалось у тех, кто ведёт учёт подробнее. Здесь стояло
     * «границы 300–600» — числа, которых в коде нет с тех пор, как рамка
     * переехала в общий реестр правдоподобия. Комментарий с числами,
     * которые он не берёт из кода, устаревает молча и уводит за собой
     * витрину: в разбор «Воспроизводство» эти же 300 и 600 были
     * переписаны отсюда.
     */
    intervals as (
      select k.animal_id,
