...
export async function reproduction(
  ...
  const res = await pool.query(
     ...
     */
    /*
     * Межотельный период — только между отёлами.
     *
     * Аборт и запуск лежат в той же таблице и различаются типом события,
     * и здесь они не отсекались: у хозяйства, ведущего аборты записями,
     * промежуток «отёл — аборт» уходил в межотельный как настоящий.
     * Рядом сервис-период и выгрузка во ФГИАС ПР их отбрасывают явно,
     * то есть одна и та же беда была починена в двух местах из трёх.
     *
     * Границы 300–600 ниже часть таких промежутков гасили, и потому
     * ошибка не бросалась в глаза: число оставалось правдоподобным
     * и просто занижалось у тех, кто ведёт учёт подробнее.
     */
    intervals as (
      select k.animal_id,
