 ...
 * а мы дописываем к ним ограничения. В разработке этого достаточно:
 * push приводит базу к построенной схеме.
 *
 * На проде — нет. Ограничение, добавленное сюда, обязано приехать
 * миграцией, написанной руками: `payload migrate:create` в этом дереве
 * не работает, потому что снимки схемы drizzle отстали (разбор — в шапке
 * `src/migrations/index.ts`). Забыть миграцию значит развести разработку
 * и прод: у себя правило есть, на проде нет, и данные, которые оно
 * не пустило бы, туда попадут.
 */
