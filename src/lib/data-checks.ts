...
import { AGE_GROUPS, labelOf } from '@/lib/dictionaries'
...
function localIssues(a: Animal, t: Thresholds): Issue[] {
   ...
   */
  /*
   * Пол и возрастная группа противоречат друг другу.
   *
   * На живой базе нашлись семь животных мужского пола с группой
   * «Корова 2 лактации». Раньше их прикрывало третье поле — «Тип
   * животного», где стояло «бык», — и расхождение выглядело мелочью
   * оформления. Поле убрано решением №228, и вопрос встал прямо: перед
   * нами бык с неверной группой или корова с неверным полом?
   *
   * Ответа у системы нет, и придумывать его она не станет. Записать
   * «раз пол мужской, ставим группу „бык“» значило бы починить половину
   * случаев и молча испортить вторую — те, где ошибся как раз пол.
   * Поэтому не правка, а находка: два значения, которые вместе
   * существовать не могут, названы человеку, а решает он.
   *
   * Тот же довод, что у единиц измерения в разборе значений (решение
   * №226): угадать можно, но угадавшая неверно и промолчавшая система
   * хуже спросившей.
   */
  if (a.sex === 'male' && ['firstCalf', 'cow2', 'cow3'].includes(a.ageGroup ?? '')) {
    push(
      'age-group-vs-sex',
      `Пол мужской, а возрастная группа — «${labelOf(AGE_GROUPS, a.ageGroup)}». Вместе эти значения ` +
        'существовать не могут: либо в поле пола ошибка, либо в группе. Исправьте то, которое неверно',
      'ageGroup',
    )
  }
...
async function relationalIssues(
  ...
  if (females.length) {
    ...
    for (const [animalId, rowsRaw] of byAnimal) {
      ...
      /*
       * Повтор номера отёла — любого, а не только первого.
       *
       * Правило `duplicate-first-calving` ниже смотрит только на номер
       * один, а `calving-number-gap` — только на дыры внутри ряда.
       * Два третьих отёла при целом ряде невидимы обоим: единица одна,
       * пропусков нет. На живой базе так оказалось у двухсот тринадцати
       * животных — четырнадцать процентов книги, — и обе проверки при этом
       * молчали, а ревизия писала «данных, на которых их видно, в базе
       * нет».
       *
       * Отдельным правилом, а не расширением `duplicate-first-calving`:
       * у первого отёла своя цена — от него считается возраст первого
       * отёла, — и смешивать «сбит учёт с самого начала» с «где-то
       * в середине задвоилось» значило бы потерять разницу, по которой
       * их чинят по-разному.
       *
       * Номер отёла определяет, сколько отёлов у коровы, а от этого зависят
       * номер лактации при загрузке доек и возрастная группа (решение
       * №228). Пока повторы не разобраны, оба числа считаются по неверному
       * основанию.
       */
      const byNumber = new Map<number, number>()
      for (const r of rowsRaw) {
        if (typeof r.number !== 'number') continue
        byNumber.set(r.number, (byNumber.get(r.number) ?? 0) + 1)
      }
      const repeated = [...byNumber.entries()].filter(([, n]) => n > 1)

      if (repeated.length) {
        push(
          'duplicate-calving-number',
          `Номер отёла повторяется: ${repeated
            .map(([num, n]) => `№${num} — ${n} записи`)
            .join(', ')}. Номер сквозной по жизни коровы, повторяться он не может — ` +
            'проверьте, не задвоилась ли загрузка и тому ли животному записан отёл',
          'calvings',
        )
      }
