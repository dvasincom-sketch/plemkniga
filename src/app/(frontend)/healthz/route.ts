 ...
 */
/**
 * Состояние пула соединений с базой.
 *
 * ## Почему это здесь появилось
 *
 * В логе прода лежит сто восемьдесят семь ошибок «timeout exceeded when
 * trying to connect» подряд, после чего сайт открылся сам. Такая запись
 * означает **две совершенно разные беды**, и лечатся они противоположно:
 *
 * — база недоступна: сеть, файрвол, перезапуск управляемого сервера;
 * — база доступна, но пул исчерпан: свободных клиентов нет, и запрос
 *   ждёт освобождения до истечения срока.
 *
 * Отличить их по тексту ошибки нельзя — он один и тот же. Отличить можно
 * по счётчикам: при исчерпанном пуле `total` упирается в `max`, а `waiting`
 * больше нуля; при недоступной базе `total` равен нулю или единице.
 *
 * Разница дорогая. Первую беду чинят с провайдером, вторую — числом
 * в переменной окружения, и час, потраченный на разговор с провайдером
 * при исчерпанном пуле, мы уже платили однажды (разбор `DATABASE_POOL_MAX`
 * в `payload.config.ts`).
 *
 * ## Почему счётчики, а не «всё хорошо»
 *
 * Проба отвечает в тот момент, когда база **ответила**, — то есть беды
 * уже нет. Счётчики говорят не о моменте ответа, а об устройстве: виден
 * потолок пула и то, сколько из него занято прямо сейчас. Заниженный
 * потолок опознаётся до следующего всплеска, а не после.
 */
const poolStats = (payload: { db?: { pool?: unknown } }) => {
  ...
  return {
    /** Потолок. `null` — умолчание библиотеки (десять клиентов). */
    max: pool.options?.max ?? null,
    /** Открыто соединений всего. */
    total: pool.totalCount ?? null,
    /** Из них простаивает. */
    idle: pool.idleCount ?? null,
    /**
     * Запросов в очереди за свободным клиентом. Больше нуля здесь
     * и есть подпись исчерпанного пула.
     */
...
export async function GET() {
  ...
  try {
    ...
    return json({
      status: 'ok',
      service: 'plemkniga',
      database: { connected: true, animals: totalDocs, pool: poolStats(payload) },
      build: buildInfo(),
      migrations,
      env,
      visibleEnv,
