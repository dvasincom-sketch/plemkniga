...
/**
 * Страница отказа собирается на запрос, а не заранее.
 *
 * Без этого Next собирал её один раз при сборке, и обращение к заголовку
 * `host` возвращало пустоту: домен неизвестен, значит «не витрина»,
 * значит книга. На `plem.online` человек, ошибшийся адресом, получал
 * знак голштинской ассоциации, поиск животного по номеру и ссылки
 * в кабинет, которого на этом домене нет вовсе, — то есть ровно ту беду,
 * ради которой эта развилка и написана.
 *
 * Найдено обходом живого сайта: код был верным, а страница отдавалась
 * заранее собранной. Такое чтением кода не ловится — только открытием
 * несуществующего адреса на боевом домене.
 */
