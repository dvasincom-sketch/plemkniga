...
export async function generateMetadata({
  ...
  return {
    /*
     * Заголовок целиком свой, без хвоста от раскладки. Хвост — это
     * «Племенная книга», и на главной он приклеивался к строке, которая
     * тем же словом и начинается: «ПЛЕМ online — племенная книга
     * молочного скота — Племенная книга». На всех прочих страницах хвост
     * нужен, потому что там стоит имя раздела; здесь стоит имя продукта,
     * и повторять за ним нечего.
     */
      ...
      <main className="container-page pb-8">
        ...
        */}
        <section className="mt-14 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {s.proof.map((p) => {
            /*
             * Адрес приходит вместе с числом, из набора строк. Список
             * адресов рядом со страницей был индексным и разъехался
             * с числами, как только у каждого языка появились свои, —
             * разбор в `lib/i18n/site-messages.ts`.
             */
            const href = p.href
            const inner = (
              <>
