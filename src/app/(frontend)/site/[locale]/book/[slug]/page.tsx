...
export async function generateMetadata({
   ...
   */
  /*
   * Заголовок — одно имя раздела, без хвоста «что внутри племенной
   * книги». Хвост приклеивает раскладка, и вместе получалось
   * «Родословная — что внутри племенной книги — Племенная книга»:
   * два слова из трёх повторяли друг друга и съедали место в выдаче,
   * где показывают около шестидесяти знаков.
   *
   * Адрес отдаётся без языка, а язык отдельно: тогда сюда встают
   * не только основной адрес, но и пять языковых копий, которые эти
   * страницы прежде не объявляли.
   */
  return pageMetadata({
    title: feature.title,
