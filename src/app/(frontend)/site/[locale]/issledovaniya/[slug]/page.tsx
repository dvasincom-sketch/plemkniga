...
export async function generateMetadata({
  ...
  /*
   * Число требований стоит в описании, а не только на странице: в выдаче
   * это единственное место, где видно, что страница кончается задачами,
   * а не перечнем того, чего у нас нет.
   */
  return pageMetadata({
    title: study.title,
    description: `${study.lead} Требований к книге: ${study.demands.length}.`,
 ...
 * Здесь текст один и тот же по устройству: семь частей, список полей,
 * счёт и требования к книге. Заведи мы по файлу на работу — семь
 * заголовков разошлись бы на третьей странице, а порядок частей на пятой,
 * и жанр перестал бы быть жанром.
 *
 * Отсюда же и то, что страница не выбирает, показывать ли требования.
 * Обвязка печатает их всегда и последними, а реестр обязан их иметь:
 * страница, из которой не следует ни одной задачи книге, в разделе
 * не заводится.
 *
