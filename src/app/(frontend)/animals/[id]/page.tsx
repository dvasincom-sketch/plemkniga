...
/**
 * Уровень оценки по документу (шкала 1…5) — это не уровень достоверности
 * записи в шапке (шкала −1…3) и не надёжность R в таблице рядом. Три величины
 * названы тремя словами намеренно: пока все три звались достоверностью,
 * читатель складывал их в одно число, которого не существует.
 */
const ReliabilityNote = ({ value }: { value?: number | null }) => (
  <span className="inline-flex items-center gap-1.5 text-[13px] leading-none text-ink-500">
    ...
    <InfoTip label="Откуда взялась эта пятибалльная оценка">
      <p className="mb-2 font-medium text-ink-900">Уровень оценки по документу</p>
      <p className="mb-2">
...
export default async function AnimalPage({
  ...
  try {
    ...
    /*
     * «Не найдено» — только когда вправду не найдено.
     *
     * Payload бросает и на отсутствующей записи, и на закрытой правами,
     * и на сбое базы; общий `catch` превращал всё это в «страница
     * не найдена». Владелец, у которого база моргнула, читал, что его
     * коровы в книге нет, — и шёл заводить её заново.
     *
     * Отсутствие и запрет по-прежнему дают «не найдено»: сказать гостю,
     * что запись есть, но закрыта, значит подтвердить существование
     * номера, который он подобрал. Всё остальное — отказ страницы,
     * и её обработчик ошибок предлагает повторить.
     */
   ...
   *
   * Считается только владельцу и только когда он раскрыл сам блок:
   * пять счётчиков и три запроса заслона на каждом открытии карточки
   * были бы платой ни за что — цифры нужны ровно в тот момент, когда
   * человек смотрит на кнопку «в архив».
   *
   * Условием была вкладка «Общие данные»: там блок и стоял. Блок уехал
   * в управление записью, а условие осталось — и раскрытый архив
   * показывал пустую рамку с заголовком. Ошибка тихая: разметка на месте,
   * данных нет, и никакой ошибки в консоли. Условие показа и условие
   * загрузки должны совпадать буквально, иначе они разойдутся при первом
   * же переезде блока.
   */
  const archiveFacts =
    isMine && manage === 'archive'
      ? await (async () => {
        ...
        {tab === 'evaluation' && maySee('evaluation') && (
          <>
            ...
            <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <div className="space-y-6">
                ...
                */}
                ...
                {/*
                   Наличие спермопродукции — отдельным блоком за оценкой
                   семени, а не строкой в нём.

                   Выше — измерение: оплодотворяющая способность, признак
                   самого быка. Здесь — состояние склада: есть ли семя
                   сегодня и чьё оно. Обе величины про одно и то же семя,
                   но отвечают на разные вопросы, и смешивать их в одной
                   таблице значило бы поставить «есть в наличии» в колонку
                   «Значение» рядом с «+2,1 п.п.».

                   Блок показывается и тогда, когда способность
                   не измерена: семя может лежать в баллоне у быка,
                   которого никто не оценивал.
                */}
                {isBull &&
                  (animal.semen?.stock?.code ||
                    animal.semen?.stock?.updatedAt ||
                    animal.semen?.stock?.available) && (
                    <Collapsible
                      title="Наличие спермопродукции"
                      note="Состояние склада, а не оценка: что есть в баллоне и чьё оно"
                    >
                      <table className="metric-table min-w-[380px]">
                        <tbody>
                          <tr>
                            ...
                            <td>
                              {/*
                                 «Нет» — такой же ответ, как «есть», и пишется
                                 словом. Пустая ячейка означала бы, что
                                 о складе ничего не известно, а это другое.
                              */}
                              {animal.semen.stock.available ? 'Есть в наличии' : 'Нет в наличии'}
                            </td>
                          </tr>
                          <tr>
                            <td>Семенной код</td>
                            <td>{animal.semen.stock.code || '—'}</td>
                          </tr>
                          <tr>
                            <td>Дата обновления</td>
                            <td className="whitespace-nowrap">
                              {dateRu(animal.semen.stock.updatedAt)}
                            </td>
                          </tr>
                          <tr>
                            <td>Собственник семени</td>
                            <td>
                              {typeof animal.semen.stock.owner === 'object' &&
                              animal.semen.stock.owner
                                ? (animal.semen.stock.owner.name ?? '—')
                                : '—'}
                            </td>
                          </tr>
                        </tbody>
                      ...
                      <p className="mt-3 max-w-[70ch] text-[13px] leading-relaxed text-ink-500">
                        Собственник семени может не совпадать с владельцем быка: бык стоит
                        на племпредприятии, а семя расходится по дистрибьюторам. Дата
                        обновления говорит, на какой день верно «есть» или «нет», —
                        без неё утверждение о складе протухает молча.
                      </p>
                    </Collapsible>
                  )}
            ...
            */}
            ...
            {/*
               Выставки — ниже фенотипа и внутри вкладки «Оценка».

               В «Событиях» лежит то, что происходит со стадом: отёлы,
               перемещения, случаи болезни. Выставка происходит не со
               стадом — это суждение о конкретном животном, высказанное
               людьми и записанное в наградах. По смыслу ей стоять рядом
               с линейной оценкой и комплексным классом.

               Быку показывается наравне с коровой, в отличие от таблицы
               лактаций: на выставки водят и быков, и это не свойство пола.

               Пустой раздел не показывается вовсе. Свёрнутый заголовок
               «Выставки» без строк читается как «данные потерялись»
               и заставляет искать, кто их не завёл, — тогда как
               у большинства животных выставок не бывает по жизни.
            */}
            {!!animal.shows?.length && (
              <section className="mt-6">
                <Collapsible
                  title="Выставки и соревнования"
                  note="Что животное показало на людях: награды и призы, как записано в документах"
                >
                  <div className="overflow-x-auto">
                    <table className="metric-table min-w-[720px]">
                      ...
                      <tbody>
                        {/*
                           Сортировка от свежего к старому: у выставок
                           спрашивают «что взяли в последний раз»,
                           а не «с чего начинали». Копия перед сортировкой —
                           `sort` меняет массив на месте, а это данные
                           из документа, которые ниже могут понадобиться
                           в исходном порядке.
                        */}
                        {[...animal.shows]
                          .sort((a, b) => String(b.date ?? '').localeCompare(String(a.date ?? '')))
                          .map((show) => (
                            <tr key={show.id ?? `${show.date}-${show.title}`}>
                              <td className="whitespace-nowrap">{dateRu(show.date)}</td>
                              <td>{show.title || '—'}</td>
                              <td>{show.place || '—'}</td>
                              <td>{show.awards || '—'}</td>
                              <td>{show.prize || '—'}</td>
            ...
            */}
            <AnimalEditBlock
              ...
              extras={[
                ...
                /*
                 * Здесь стояло «GUID (ФГИАС ПР)» — и это было неправдой
                 * ровно в том месте, где её читают. `uuid` выдаёт
                 * `randomUUID()` при создании карточки; реестр о нём
                 * не знает ничего. Подпись переименована в коллекции,
                 * а на карточке осталась старой — это второй экземпляр
                 * одного и того же названия, и разошлись они, как всегда
                 * расходятся два экземпляра.
                 *
                 * Цена путаницы: человек, переписывающий этот номер
                 * в шаблон реестра в колонку «Базовый номер», заводит
                 * там второе животное рядом с настоящим.
                 */
                { label: 'Идентификатор учётной системы', value: animal.uuid || '' },
                /*
                 * Настоящие номера реестра — рядом, чтобы их было видно
                 * и чтобы отличие от нашего ключа читалось глазами,
                 * а не выяснялось на отказе файла. Пустые они у всех,
