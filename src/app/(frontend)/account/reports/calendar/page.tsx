...
/**
 * Календарь: запуск, отёл, проверка стельности, повторное осеменение.
 *
 * ## Почему списки, а не один поток событий
 *
 ...
 * проверку стельности назначают на день приезда врача, а осеменять заново
 * зовут по циклу. Списки можно распечатать по отдельности и отдать разным
 * людям — одну ленту нельзя.
 *
 * Списков четыре, и число здесь не пишется словом: четвёртый — «Осеменить
 * заново» — добавился позже, а тексты вокруг ещё месяцами говорили «три».
 *
 * ## Пустой список — не поломка
 *
 * Все четыре складываются из осеменений. Хозяйство, которое их не
 * записывает, увидит здесь пусто, и это правда: без даты осеменения срок
 * отёла не выводится ниоткуда. Поэтому пустота объясняется словами,
 * а не молчит.
 */
  ...
  return (
    <>
      ...
      <CabinetPage
        ...
        intro={
          <>
            Кого запускать, кто телится, кого проверять на стельность и кого осеменять
            заново. Всё выводится из отёлов и осеменений, которые вы уже
      ...
      >
        {nothing && (
          <div className="card mt-8">
            <p className="max-w-[80ch] text-[15px] leading-relaxed text-ink-700">
              Календарь пуст, и это не поломка. Все списки строятся от даты
        ...
        {/*
           Когда пусто всё, объяснение выше сказано один раз — карточки
           «никого нет» под ним только повторяли бы его. Когда пуст
           один список из четырёх, его собственная пустота содержательна:
