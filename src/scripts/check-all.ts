...
import { CHECKS, CHECK_WRITES } from '@/lib/check-registry'
 ...
 * ## Зачем
 *
 * Ручка `/checks` гоняет только пробы — те, что ничего не пишут
 * и не ходят наружу; остальные запускались по одной, руками, и на
 * странице «Статус» честно значились как «не гонялись». Полсотни команд
 * подряд — это работа, которую не делают: её откладывают до случая,
 * а случаем оказывается поломка.
 *
 * Числа здесь не пишутся словами: они уже отставали втрое. Сколько чего
 * считает сам реестр (`PROBE_COUNT`, `CHECK_WRITES`, `CHECK_SERVER`).
 *
 ...
 *
 * `BASE` нужен тем проверкам, которые ходят по страницам снаружи:
 * обход, ссылки навигации, страница документации, живой обмен. Без него
 * они пойдут на localhost и либо пропустятся, либо проверят не то.
 *
 *   npm run check:all
 ...
 *
 * Нужен ровно для одного случая — прогона по боевой базе. Все запустить
 * там нельзя и не будет можно: часть заводит записи, и обрыв посреди
 * прогона оставит в книге мусор, неотличимый от настоящих данных.
 * Сколько именно пишет и сколько читает, считает сам реестр
 * (`CHECK_WRITES`); написанное словами тут уже отставало.
 */
...
function runOne(code: string): Promise<Outcome> {
  ...
  return new Promise((resolve) => {
    ...
    child.on('close', (codeExit, signal) => {
       ...
       * но соблюдается во всех; свести их к общему виду стоило бы правки
       * в каждом скрипте ради того же результата.
       *
...
async function main() {
  ...
  const plan = CHECKS.filter((c) => {
    ...
    /*
     * Отчёты в прогон не идут: они не могут упасть, и зелёная строка
     * от них означала бы «проверено» там, где просто напечатано.
     * Названы они ниже поимённо — чтобы не выпали из виду совсем.
     */
    if (c.report) return false
    return true
  })

  if (READONLY) {
    console.log(`Только читающие: ${CHECK_WRITES} пишущих проверок пропущено.`)
  }
  if (!local) {
    console.log('База не местная: проверки о собственном процессе пропущены.')
  }

  const reports = CHECKS.filter((c) => c.report)
  console.log(
    `\nПрогон «${LABEL}»: проверок ${plan.length}` +
      (reports.length
        ? `; отчётов, которые читают глазами и в прогон не входят: ${reports.length} ` +
          `(${reports.map((r) => r.code).join(', ')})`
        : '') +
      '\n',
  )
  ...
  /*
   * Пропуск — не удача, и в коде возврата тоже.
   *
   * На страницу это записывалось верно (`ok: failed === 0 && skipped === 0`),
   * а наружу прогон отдавал ноль: тот, кто гоняет из cron или из сборки
   * и смотрит на код возврата, считал прошедшим прогон, где пять проверок
   * страниц пропущены из-за лежащего сервера.
   */
