 ...
 *
 * Здесь только те, что записаны кириллицей: российский нормативный акт
 * и российское ведомство переводить нельзя (`docs/lokalizatsiya.md`),
 * а совпадение с русским у них — решение, а не недоделка.
 *
 * Латинских имён в списке нет намеренно, и это правка после первого же
 * прогона. Сначала здесь стояли `ICAR`, `ISO`, `ADE`, `JSON` и прочие,
 * и проверка тут же нашла `wiki.icar.org`, `ISAG` и `World Holstein
 * Friesian Federation`: список по определению отстаёт от кода, потому
 * что растёт по одному имени за прогон, и каждое такое имя — красная
 * строка в отчёте до тех пор, пока кто-то не допишет список.
 *
 * Признак вместо списка: строка без кириллицы и без армянского письма
 * не может быть непереведённым русским текстом — это имя, адрес или
 * код. Проверка от этого не слепнет: пропущенный перевод всегда
 * написан буквами того языка, с которого не переведён.
 */
...
/** Латиница, цифры и знаки: имя, адрес или код, а не текст. */
const noSourceScript = (s: string): boolean => !/[А-Яа-яЁё԰-֏]/.test(s)
