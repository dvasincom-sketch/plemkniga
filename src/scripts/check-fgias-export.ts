...
function mainSheet() {
  ...
  /*
   * Место рождения и мелкие поля — то, ради чего заводились три
   * территориальных справочника. Утверждения написаны на значения,
   * а не на «поле есть»: колонка, которая приняла ключ и положила его
   * не туда, выглядит заполненной.
   */
  console.log('\nОсновные сведения: место рождения и даты определения\n')

  const full = buildMain([
    {
      ...animal,
      breedTypeUuid: '18e45cfe-48c9-4e40-aea5-4ae1bf2bc033',
      breedDate: '2021-04-01T00:00:00.000Z',
      purposeDate: '2021-04-01T00:00:00.000Z',
      receiptMethodUuid: '2e9cce84-bc33-4e86-b2d9-464655c8ff19',
      birthCountryUuid: 'b425442a-d055-43e1-9931-1d3b6d5592cd',
      birthRegionUuid: '133a2628-1d32-435b-b65e-dbfd9404075b',
      birthDistrictUuid: '2e1cce84-bc33-4e86-b2d9-464655c8ff16',
      birthFarm: { name: 'ООО «Родное»', inn: '3924003136', kpp: '392401001' },
    },
  ])
  const r2 = full.rows[0]!
  const at2 = (title: string) => r2[MAIN_COLUMNS.findIndex((c) => c.title === title)]

  check(at2('Страна рождения') === 'b425442a-d055-43e1-9931-1d3b6d5592cd', 'страна рождения ключом')
  check(at2('Регион рождения') === '133a2628-1d32-435b-b65e-dbfd9404075b', 'регион рождения ключом')
  check(at2('Район рождения') === '2e1cce84-bc33-4e86-b2d9-464655c8ff16', 'район рождения ключом')
  check(at2('Наименование хозяйства при рождении') === 'ООО «Родное»', 'хозяйство рождения названо')
  check(at2('ИНН хозяйства при рождении') === '3924003136', 'и его ИНН взят из связи')
  check(at2('Тип породы') !== '', 'тип породы уезжает отдельной колонкой от породы')
  check(at2('Дата определения породы') === '2021-04-01', 'дата определения породы')
  check(at2('Дата определения назначения') === '2021-04-01', 'дата определения назначения')
  check(at2('Способ получения') !== '', 'способ получения уезжает')

  /*
   * Хозяйство рождения, известное только названием: реквизитов у него
   * нет и взять их неоткуда. Пустая ячейка честнее любой подстановки —
   * ИНН чужого хозяйства мы не знаем и знать не можем.
   */
  const byName = buildMain([{ ...animal, birthFarm: { name: 'СПК «Заря», по свидетельству' } }])
  const r3 = byName.rows[0]!
  const at3 = (title: string) => r3[MAIN_COLUMNS.findIndex((c) => c.title === title)]
  check(
    at3('Наименование хозяйства при рождении') === 'СПК «Заря», по свидетельству',
    'хозяйство рождения по названию тоже уезжает',
  )
  check(at3('ИНН хозяйства при рождении') === '', 'а ИНН у него пуст, а не выдуман')
}
...
/**
 * У каждой колонки выгрузки есть дорога, по которой её заполняют.
 *
 * ## Откуда взялось это утверждение
 *
 * Под три шаблона экстерьера в книгу завели девять полей — и не заметили,
 * что писать в эту коллекцию вправе одна Ассоциация: `create: isAdmin`
 * стоял там со дня заведения, скопированный от соседней таблицы
 * с расчётами модели. Четыре кнопки выгрузки из двадцати обещали файлы,
 * которые всегда будут пустыми, и обещали убедительно: колонки на месте,
 * заголовки сверены с шаблоном, проверки зелёные.
 *
 * Поймать такое сверкой с реестром нельзя — со стороны реестра всё
 * верно. Ловится оно только вопросом «а кто это заполнит».
 *
 * ## Проверяется по признакам экстерьера
 *
 * Именно они выявили дыру, и именно их много: тридцать два признака
 * в трёх шкалах. Каждый должен быть колонкой в ведомости бонитёра —
 * иначе его неоткуда взять, кроме как ввести по одному в админке.
 */
function inputPath() {
  console.log('\nЧем заполняют то, что выгружается\n')

  const sheet = DATASETS.find((d) => d.key === 'exteriors')
  check(!!sheet, 'ведомость бонитёра заведена набором загрузки')
  if (!sheet) return

  const columns = new Set(sheet.groups.flatMap((g) => g.columns).map((c) => c.key))

  const needed = [
    ...LINEAR_TRAITS.filter((t) => t.key).map((t) => ({ key: t.key!, where: 'линейная оценка' })),
    ...TYPE_TRAITS.map((t) => ({ key: t.key, where: 'тип телосложения' })),
    ...BULL_TRAITS.map((t) => ({ key: t.key, where: 'оценка быка' })),
    ...YOUNG_TRAITS.map((t) => ({ key: t.key, where: 'молодняк' })),
  ]

  const missing = needed.filter((n) => !columns.has(n.key))
  check(
    missing.length === 0,
    'каждый выгружаемый признак экстерьера принимается загрузкой',
    missing.map((m) => `${m.key} (${m.where})`).join(', '),
  )

  /*
   * И дата: без неё запись об осмотре не заводится вовсе, а колонка
