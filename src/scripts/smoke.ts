...
/** Отвечает ли по адресу сервер разработки — выясняется до первого замера. */
let dev = false
...
async function main() {
  /*
   * Сперва проверяем, что по адресу вообще кто-то отвечает.
   *
   * Прогон против незапущенного сервера выглядит как разгром: тридцать
   * восемь страниц «не отвечают» и таблица самых медленных, где все
   * по нулю секунд. Читается это как поломка приложения, а означает —
   * «сервер не поднялся», и один раз мы на этом уже потеряли круг:
   * `npm run start` не встал из-за занятого порта, а прогон честно
   * померил пустоту.
   *
   * Нулевые секунды и есть подпись такой беды: страница не может
   * ответить мгновенно, а отказ в соединении может.
   */
  try {
    const ping = await fetch(`${BASE}/healthz`)
    if (!ping.ok && ping.status >= 500) {
      console.log(`\n  ✗ ${BASE} отвечает ${ping.status} — сервер поднялся, но нездоров`)
    }

    /*
     * Сервер разработки узнаётся по метке сборки в адресах его же скриптов.
     *
     * Он собирает страницу при первом обращении, и его секунды — это
     * компиляция, а не работа страницы. Разница не в проценты: у нас
     * `/account/indices/1` показывала 18,5 секунды на разработке
     * и 0,29 на прод-сборке. Полдня ушло на поиск того, чего нет,
     * и остановило нас только разложение по шагам, где данные собрались
     * за полсекунды.
     *
     * Признак — `?v=` при `main-app.js`: в прод-сборке версия зашита
     * в само имя файла, а в разработке дописывается меткой времени.
     */
    const html = await (await fetch(BASE)).text()
    dev = /main-app\.js\?v=|react-refresh/.test(html)
  } catch {
    console.log(`\n  ✗ по адресу ${BASE} никто не отвечает — сервер не запущен`)
    console.log('    Проверьте, что он поднят и что порт тот же (PORT= при npm run start).')
  ...
  if (dev) {
    console.log(
      '\n  ⚠ Это сервер разработки. Его время — это сборка страницы, а не её работа,\n' +
        '    и сравнивать эти секунды с чем-либо нельзя. Для замера скорости:\n' +
        '    npm run build && PORT=3010 npm run start, затем BASE=http://localhost:3010 npm run smoke',
    )
  }
  console.log(
