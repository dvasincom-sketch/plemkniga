...
/**
 * Приставка стенда: своя, не `99`.
 *
 * Она защищает животных стенда от `seed:bulk --drop`, но не защищала
 * его хозяйство: та уборка сносила всё, чьё имя начинается на
 * «Синтетика», а животные с чужой приставкой оставались — и удаление
 * падало внешним ключом. Там теперь точное имя, здесь — приставка;
 * нужны обе, и первая редакция комментария обещала лишнего.
 */
const PREFIX = '98'
...
async function main() {
  ...
  /*
   * Выбытие без даты. Записывается прямым запросом, и иначе теперь нельзя:
   * карточка такую запись не сохранит — состояние «выбыло» требует даты.
   * Стенду это и нужно: он показывает, как выглядит книга, в которую такие
   * записи попали до запрета, и что о них говорит проверка.
   *
   * Причина здесь проставлена намеренно, чтобы находка была одна.
   * Животное без причины и без даты дало бы две, и стенд перестал бы
   * отвечать на вопрос «работает ли именно это правило».
   */
  await plant(['disposal-date-missing'], 'выбыла, дата выбытия не проставлена', async () => {
    await animal({ state: 'culled', disposal_date: null, disposal_reason_id: reasonId })
  })
