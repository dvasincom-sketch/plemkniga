...
import { resolveDatabase, shouldPushSchema } from '@/lib/db-url'
...
/*
 * Проверка отказывается работать, пока включён push.
 *
 * Это не осторожность, а условие осмысленности. `push` приводит схему
 * базы к конфигу при старте Payload — то есть устраняет ровно то
 * расхождение, которое проверка пришла измерить. Запущенная так, она
 * всегда сообщает «сошлось», и сообщает честно: к моменту сравнения
 * действительно сошлось. Проверка, которая не может не пройти, хуже
 * отсутствующей — та хотя бы не занимает место.
 *
 * Так и было: `check:all` запускал прогоны с настройками разработчика,
 * push на локальной базе включён, и забытая миграция — та самая, ради
 * которой всё писалось, — прошла бы незамеченной.
 *
 * Отказ, а не тихое выключение push внутри себя: выключив его здесь,
 * мы бы починили эту проверку и оставили сорок семь остальных гонять
 * схему вперемешку.
 */
{
  const { driverUri } = resolveDatabase()
  if (shouldPushSchema(driverUri)) {
    console.log('Проверка схемы бессмысленна при включённом push.')
    console.log('')
    console.log('  Payload приводит схему базы к конфигу при старте, то есть устраняет')
    console.log('  расхождение до того, как мы его измерим. Ответ «сошлось» будет')
    console.log('  правдой и не будет значить ничего.')
    console.log('')
    console.log('  Запускайте так:  PAYLOAD_DB_PUSH=false npm run check:schema')
    console.log('  В общем прогоне (npm run check:all) это уже сделано.')
    process.exit(1)
  }
}
