...
/* ------------------------------------------------------------------ *
 *  2. В нерусских наборах нет отсылок к России                        *
 * ------------------------------------------------------------------ */

/**
 * Слова, которых на нерусской странице быть не должно.
 *
 * ## Почему это правило, а не вкусовщина
 *
 * Витрина обращена к хозяйствам и ассоциациям разных стран. Русскому
 * читателю сильнейший довод — срок, с которого регистрация
 * в государственном реестре обязательна. Казахскому тот же срок либо
 * навяжет чужую обязанность, либо опознается как чужой — и тогда
 * перестанут верить и остальным числам: раз одно поставили наугад,
 * значит и другие могли.
 *
 * Первая попытка это исправить была половинчатой: срок убрали, а число
 * шаблонов оставили, прямо назвав их «российскими». Вышло хуже —
 * упоминание чужого государства читается как «это решение не для нас».
 *
 * ## Почему без прогона это вернётся
 *
 * Строки пишутся по-русски и переводятся следом. Русский довод,
 * убедительный на своём месте, доезжает до чужого языка сам собой,
 * и заметить это может только тот, кто читает страницу на киргизском, —
 * то есть никто из нас.
 *
 * Список нарочно грубый: он ловит корень, а не слово целиком, потому что
 * склонения у шести языков разные. Ложное срабатывание стоит взгляда
 * человека; пропуск стоит хозяйства, решившего, что продукт не про него.
 */
/**
 * Где упоминание допустимо.
 *
 * Язык и государство — разные вещи, и проверка их не различает.
 * Оговорка о непроверенном переводе честно называет языки, на которых
 * текст вычитан: русский и английский. Это не отсылка к стране, а ответ
 * на вопрос «где посмотреть точную формулировку», и убрать его значило бы
 * оставить читателя без запасного варианта ради чистоты правила.
 */
const ALLOWED_PATHS = new Set(['draft.notice'])

const RUSSIA_MARKERS = [
  'росси', // русский, киргизский: Россия, российский, Россиянын
  'russia', // английский, а заодно домен holstein-russia
  'фгиас',
  'fgias',
  'ресей', // казахский
  'расі', // белорусский
...
for (const [setName, byLocale] of SETS) {
  for (const info of LOCALES) {
    ...
    for (const [path, value] of stringsOf(byLocale[info.code])) {
      ...
      const hit = RUSSIA_MARKERS.find((w) => lower.includes(w))
      if (hit) {
        fails.push(
          `${setName}, ${info.code}: строка ${path} упоминает Россию («${hit}»). ` +
            `На нерусской витрине довод должен быть международным: чужое государство ` +
            `читается как «это решение не про нас»`,
        )
      }
    }
  }
...
console.log(`Проверено отсылок к России в ${LOCALES.length - 1} нерусских наборах: ни одной`)

/* ------------------------------------------------------------------ *
 *  3. Разбор Accept-Language                                          *
 * ------------------------------------------------------------------ */
