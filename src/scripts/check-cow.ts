...
async function main() {
  ...
  /*
   * Блок появился из-за ложной тревоги и оставлен из-за настоящей нужды.
   *
   * Два прогона подряд дали несводимые числа: отёлов поровну, а животных,
   * которым они принадлежат, всё меньше. Я объявил это порчей данных —
   * а виновато было листание этого же скрипта без сортировки по `id`
   * (решение №229): одни строки приходили дважды, другие не приходили.
   * Числа сошлись, как только листание починили.
   *
   * Блок остался, потому что вопрос он задаёт правильный, и однажды
   * ответит не нулём. Правило «отёл повышает группу» опирается на то,
   * что отёл принадлежит тому животному, на которое указывает; если
   * указание съедет по-настоящему, правило аккуратно перенесёт ошибку
   * в карточки и заверит её датой определения.
   *
   * Повтор номера — главный признак: номер отёла сквозной по жизни
   * коровы и повторяться не может. Ту же беду ищет правило
   * `duplicate-calving-number` в разборе пакета; здесь она считается
   * по всей книге разом, а не по выборке.
   */
