 ...
 *
 * Разборы, исследования и породные страницы объявлены русскими в самом
 * коде (`lib/notes.ts`, `lib/studies.ts`, `lib/breed-pages.ts`): шесть
 * языков от текста, вся ценность которого в точности формулировок, дали
 * бы пять машинных переводов. «Эволюция продукта» — та же история,
 * и адрес у неё без языка.
 *
