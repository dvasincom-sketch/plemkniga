...
/*
 * Приставка проверяется сразу, а не всплывает ошибкой посреди прогона.
 *
 * Из неё складываются и номер животного, и uuid, и оба уникальны
 * в базе. Приставка на три знака сдвинула бы номер за двенадцать цифр,
 * на один — столкнулась бы с чужим набором. Сказать об этом надо
 * до того, как в базу ушла первая строка.
 */
if (!/^\d{2}$/.test(PREFIX)) {
  console.error(`\nПриставка --tag должна быть из двух цифр, а не «${PREFIX}».\n`)
  process.exit(1)
}
...
function animalRow(opts: {
  ...
  return {
    ...
    row: [
      ...
      'rf',
      /*
       * uuid обычно ставит хук; здесь его нет, а поле уникально
       * и обязательно.
       *
       * Приставка входит в него наравне с номером. Без неё uuid
       * складывался из одного счётчика, счётчик у каждого набора
       * свой и начинается с единицы — и второй набор падал
       * на `animals_uuid_idx`, столкнувшись с первым. Уникальность
       * номера при этом соблюдалась: в номер приставка входила
       * с самого начала, а в uuid её забыли.
       */
      `${PREFIX}${'9'.repeat(6)}-0000-4000-8000-${String(n).padStart(12, '0')}`.slice(0, 36),
      name,
      name,
      male ? 'bull' : 'cow',
      sex,
...
async function generate(client: PoolClient) {
  ...
  /*
   * Номера заведённых хозяйств печатаются вслух.
   *
   * Без них человек, только что собравший стадо, идёт проверять его
   * прогоном — и указывает номер наугад. Так и вышло: прогон ушёл
   * на демо-ферму под номером один, честно показал её числа, и выглядело
   * это как проверка нового стада. Проверка не на тех данных хуже
   * отсутствия проверки: она отвечает уверенно и не о том.
   *
   * Печатается и готовая команда: подсказка, которую можно скопировать,
   * не оставляет места догадке.
   */
  const names = ctx.orgs.length === 1 ? 'Хозяйство' : 'Хозяйства'
  console.log(`\n${names}: ${ctx.orgs.map((id) => `#${id}`).join(', ')}`)
  ...
  console.log(
    '\nДальше:\n' +
      ...
      `  npm run check:herd -- --org=${ctx.orgs[0]}   — отчёты по этому стаду`,
  )
}
  ...
  for (const [i, cow] of cows.entries()) {
    ...
    for (const [k, calvingDate] of cow.calvings.entries()) {
      ...
      calvings.push([
        id, l, calvingDate,
        /*
         * Тип рождения, а не пол: пол ушёл в отдельные числа, и заводить
         * их здесь пришлось бы третьей колонкой в этот массив. Стенду
         * на семьсот коров они не нужны — им проверяют скорость,
         * а не отчёт по приплоду.
         */
        pick(['one', 'one', 'twins']),
        int(280, 340),
