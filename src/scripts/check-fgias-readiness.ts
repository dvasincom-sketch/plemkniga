 ...
 *
 * Стоило это ложной тревоги. Скрипт объявил, что у двухсот тринадцати
 * животных — четырнадцати процентов книги — есть два отёла с одним
 * номером, и что число хозяев у отёлов падает от прогона к прогону.
 * И то и другое было артефактом листания: одна строка, дважды попавшая
 * в выборку, выглядела как задвоенный отёл, а потерянные строки —
 * как животные, лишившиеся отёлов. У `animals` то же самое по другой
 * причине: сид проставляет всем записям одинаковый `created_at`,
 * и сортировка по умолчанию их не различает.
 *
 * `id` уникален по построению, и порядок по нему один и тот же всегда.
 */
async function all(
...
async function main() {
  ...
  console.log(`\n${'─'.repeat(72)}`)
  /*
   * Список того, чего в книге нет, — самое стареющее место этого отчёта.
   *
   * Он был написан один раз и не пересматривался: к сентябрю 2026 года
   * пять пунктов из шести уже заведены — живая масса стала коллекцией
   * `weighings`, выставки таблицей `shows`, экстерьер молодняка и типы
   * телосложения полями `animal-exteriors`, наличие спермопродукции
   * группой `semen.stock`, УНСМ и базовый uuid — группой `fgias`.
   * Отчёт продолжал называть их отсутствующими, и читатель, который
   * поверил, шёл заводить заведённое.
   *
   * Поэтому список короткий и с оговоркой: полноту колонок считает сам
   * прогон выше, а здесь только то, чего действительно нет ни в одном
   * поле книги.
   */
  console.log('Чего в книге нет вовсе — считать нечего, названо для полноты картины')
  console.log('')
  for (const s of [
    'Генотип животного — хранится результат теста, а не сами вызовы по маркерам',
    'Групповые события: обработки, вакцинации, кормление — реестр их не просит, книга не ведёт',
  ])
    console.log(`  · ${s}`)
