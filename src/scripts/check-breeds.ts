...
/*
 * У каждой страницы породы имя из справочника реестра должно совпасть
 * с именем в каталоге.
 *
 * Совпадение это не формальность: по имени страница берёт состояние —
 * код ICAR, ключ реестра, ведётся ли книга. Не совпало — страница
 * молча покажет «породы нет в выписке» там, где порода есть, и вместо
 * главного своего раздела отдаст одну строку извинений.
 *
 * Первым же прогоном поймана «Чёрно-пёстрая»: в реестре она записана
 * без «ё», и глазами это неотличимо.
 */
{
  const { BREED_PAGES } = await import('@/lib/breed-pages')
  const names = new Set(rows.map((r) => r.name))

  for (const page of BREED_PAGES) {
    if (!names.has(page.registryName)) {
      fail(
        `страница «${page.title}»: имени «${page.registryName}» нет в справочнике реестра — ` +
          'состояние породы на ней не покажется',
      )
    }
  }
}
