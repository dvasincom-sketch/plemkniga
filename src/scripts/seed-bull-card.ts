...
async function main() {
  ...
  const data = {
    ...
    /*
     * Источник привезённой оценки — часть эталона, а не украшение.
     *
     * Безымянная чужая оценка нечитаема: индекс из американского каталога
     * и индекс областного центра — разные величины. Образец, показывающий
     * оценку без имени, учит оставлять её без имени; заодно без источника
     * не работает проверка расхождения с расчётом книги — ей нечего
     * назвать в объяснении.
     *
     * Быки почти всегда приезжают с импортной оценкой вместе с семенем,
     * поэтому здесь CDCB.
     */
    ipcDetails: {
      ...(bull.ipcDetails ?? {}),
  ...
  console.log(`  источник привезённой оценки: CDCB (США), база CDCB-2025`)
