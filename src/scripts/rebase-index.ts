...
/** Значение признака у животного — по тому же пути, что и в расчёте. */
const readValue = (animal: Animal, path: string): number | null => {
  ...
  if (node && typeof node === 'object') {
    const f = (node as Record<string, unknown>).forecast
...
async function main() {
   ...
   */
  const samples = new Map<TraitKey, number[]>(TRAIT_BASE.map((t) => [t.key, []]))
  let animals = 0
  let page = 1
  ...
  for (;;) {
    ...
    for (const a of batch.docs as Animal[]) {
      ...
      for (const t of TRAIT_BASE) {
        const v = readValue(a, t.path)
        if (v !== null) samples.get(t.key)!.push(v)
      }
    }
  ...
  console.log(
    pad('Признак', 32) + pad('Оценок', 9) + pad('Среднее', 11) + pad('σ своя', 11) + 'σ заимств.',
  )
  ...
  for (const r of rows) {
    console.log(
      pad(`${r.trait.label}, ${r.trait.unit}`, 32) +
        pad(String(r.n), 9) +
        pad(num(r.mean), 11) +
        pad(r.sd === null ? 'мало данных' : num(r.sd), 11) +
        num(r.trait.sd),
    )
  }
