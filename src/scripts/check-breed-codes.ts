...
import { ICAR_BREEDS } from '@/lib/breeds-catalog'
...
async function main() {
  ...
  /*
   * Код существует в списке Interbull, а не просто выглядит как код.
   *
   * Этой проверки не было, и она нашла в засеваемом справочнике два
   * выдуманных кода: `AYR` у айрширской и `BLW` у чёрно-пёстрой. Оба
   * правильной формы, оба проходят сборку номера, и обоих нет в списке
   * вовсе — айрширская у Interbull входит в `RDC`, а у чёрно-пёстрой
   * своей строки нет и быть не должно (`RU_TO_ICAR` её намеренно
   * не сопоставляет).
   *
   * Опасность тут не косметическая: код из справочника сильнее нашего
   * сопоставления по имени, и он уезжает в обмен полем `primaryBreed`
   * и в международный номер животного. Выдуманный код объявляет чужой
   * стране породу, которой нет, — и объявляет уверенно, потому что
   * форма у него верная.
   */
  const known = new Map(ICAR_BREEDS.map((b) => [b.code, b.name]))

  for (const r of withCode) {
    const code = r.whff_code!.trim().toUpperCase()
    if (known.has(code)) continue
    fails.push(
      `порода «${r.name}»: кода «${code}» нет в списке Interbull — ` +
        `в обмен уедет порода, которой в справочнике не существует`,
    )
  }
