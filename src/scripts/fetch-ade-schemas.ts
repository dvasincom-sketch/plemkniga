 ...
 *
 * `docs` добавлен не для полноты. Обмен наборами — половина пятнадцатого
 * раздела — описан **только** там: ни в схемах, ни в схемах адресов
 * его нет вовсе. Пока каталог не переносился, у нас лежала подробная
 * копия того, что можно проверить машиной, и ни строчки того, что задаёт
 * смысл: порядок применения изменений, устройство метки продолжения,
 * заголовок полной пересинхронизации.
 *
 * Разница дорогая. Схему можно сверить прогоном, а прозу — только
 * прочитав; и читать её надо ту, что лежит рядом с проверенной копией,
 * а не ту, что нашлась в поиске и относится к другой ветке.
 */
const KEEP = ['resources', 'types', 'enums', 'collections', 'url-schemes', 'docs']
