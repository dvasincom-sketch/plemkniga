...
import { HERD_REPORTS } from '@/lib/probes'
...
async function main() {
  ...
  if (r)
    console.log(
      /*
         Оба числа печатаются рядом намеренно. Полгода экран показывал дни
         до первого осеменения под именем сервис-периода, и заметить это
         можно было ровно так — увидев их вместе и удивившись, что второе
         меньше первого не на пару дней, а на два половых цикла.
      */
      `      сервис-период ${r.serviceperiod ?? '—'}, до первого осеменения ` +
        `${r.daysToFirstService ?? '—'}, на стельность ${r.perConception ?? '—'}, ` +
  ...
  /*
   * Список отчётов — общий с ночной пробой (`lib/probes.ts`,
   * `HERD_REPORTS`). Здесь он не повторяется, а сверяется: скрипт печатает
   * по каждому отчёту свои подробности и потому зовёт их поимённо, но
   * отчёт, добавленный к пробе и забытый здесь, обязан быть назван.
   * Пока списка было два, они разъезжались молча, и два прогона под одним
   * именем отвечали на разные вопросы.
   */
  const covered = [
  ...
  const missed = HERD_REPORTS.filter((r) => !covered.includes(r.name)).map((r) => r.name)
  check(
    missed.length === 0,
    'скрипт покрывает все отчёты ночной пробы',
    missed.join(', '),
  )
