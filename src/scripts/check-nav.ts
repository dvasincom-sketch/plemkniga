...
async function main() {
  ...
  /*
   * «Аукционы» остались страницей и ссылкой в подвале, «Быки» — экраном
   * внутри «Стада». Проверяется, что их нет именно в меню шапки, а не что
   * слов нет на странице вовсе.
   */
  ...
  check(!headerNav.includes('/bulls/compare'), 'сравнения быков в шапке нет')

  /*
   * Убрав пункт из шапки, легко вернуть экран в то состояние, ради выхода
   * из которого его туда и ставили: построен, работает, попасть неоткуда.
   * Поэтому проверяется не отсутствие, а замена — что вход есть там, куда
   * его перенесли.
   */
  const reports = pages.get('/account?tab=herd&sub=reports') ?? ''
  check(
    reports.includes('/bulls/compare'),
    'сравнение быков достижимо из «Стадо → Отчёты»',
  )
