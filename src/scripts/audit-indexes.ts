...
async function main() {
  ...
  console.log(`Счётчики обнулены: ${resetRow.rows[0]?.since ?? 'неизвестно'} (${ru(days)} дн. назад)`)
  if (warm && days < WARM_DAYS) {
    console.log(
      `\nОкно наблюдения короткое (${ru(days)} дн.). Обращений достаточно, но редкие\n` +
