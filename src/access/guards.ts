...
/**
 * Штамп хозяйства, при котором запись появилась.
 *
 * ## Зачем он, если у записи есть животное, а у животного владелец
 *
 * Пока животное не продают, владелец отвечает на всё. После продажи
 * вопросов становится два, и ответы у них разные: карточку ведёт покупатель,
 * а дойки и отёлы за прошлые пять лет собрал продавец. Отдать продавцу
 * доступ по прежнему владению нельзя — он увидит и то, что покупатель
 * записал после сделки, то есть свою бывшую корову в чужом стаде.
 * Отобрать всё тоже нельзя: это его собственный труд за годы.
 *
 * Штамп разделяет эти два ответа. Строка помнит, чьей она была в момент
 * появления, и продавец видит ровно свой период — без единой записи
 * из чужого хозяйства.
 *
 * ## Почему проставляется здесь, а не в форме
 *
 * Дойки приходят импортом тысячами, отёлы — из скриптов переноса, события —
 * через API. Штамп, проставленный формой, защищал бы только форму,
 * а пробел в нём означает строку, которую после продажи не увидит никто.
 *
 * Запрос за животным делается только при создании и только если штамп пуст:
 * импорт может проставить его сам, зная владельца пакета, и тогда лишнего
 * обращения к базе не будет вовсе.
 */
export const stampOwnerOrg: CollectionBeforeChangeHook = async ({ data, req, operation }) => {
  if (operation !== 'create') return data
  if (!data || data.ownerOrg) return data

  const animalId = relId(data.animal)
  if (!animalId) return data
  ...
  const owner = relId((animal as { owner?: unknown })?.owner)
  if (owner !== null) data.ownerOrg = owner
  return data
}
