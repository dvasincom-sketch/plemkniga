...
export const Animals: CollectionConfig = {
  ...
  fields: [
     ...
     */
    {
      ...
      index: true,
      /*
       * Обязателен — в модели, в типах и в базе одновременно.
       *
       * Вокруг поля построено обещание «неизменный уникальный ключ»:
       * по нему загрузка файлом находит свою запись, и он же уезжает
       * во ФГИАС ПР как идентификатор учётной системы. Держалось обещание
       * на одном хуке `beforeValidate`, который подставляет `randomUUID()`
       * при создании. Любой путь записи мимо него оставил бы пусто,
       * а уникальный индекс этого не замечает: в PostgreSQL пустое
       * не равно пустому, и строк без ключа может быть сколько угодно.
       *
       * Хук выполняется раньше проверки обязательности, поэтому человеку
       * в форме заполнять ничего не придётся. А вот код, создающий животное
       * в обход форм — сиды и прогоны, — обязан теперь назвать ключ явно,
       * и это правильная цена: место, заводящее животное мимо приложения,
       * должно знать, что у животного есть ключ.
       */
      required: true,
      admin: {
        readOnly: true,
