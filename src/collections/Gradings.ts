 ...
 *
 * ## `req` передаётся обязательно
 *
 * Хук работает внутри транзакции записи бонитировки, и обновление
 * карточки должно попасть в неё же. Отдельное подключение здесь даёт
 * не ошибку, а зависание до таймаута: вставка бонитировки держит на
 * строке животного блокировку по внешнему ключу, а правка карточки
 * со стороны ждёт её снятия — а снимется она только после хука.
 *
 * Это решение №20, и оно уже написано в `lib/evaluation-snapshot.ts`
 * теми же словами. Написано — и всё равно повторено здесь заново:
 * сид повис на первом же животном и висел пятнадцать минут, потому
 * что `req` не был передан. Правило, известное в одном файле, не знает
 * о себе в другом.
 *
