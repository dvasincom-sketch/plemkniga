...
export const Users: CollectionConfig = {
  ...
  auth: {
    ...
    /**
     * Заслон от перебора паролей.
     *
     * ## Почему это нужнее, чем кажется
     *
     * Вход здесь по паролю, а `POST /api/users/login` открыт всем и отвечает
     * за десятки миллисекунд. Без ограничения перебор словаря из миллиона
     * паролей упирается только в ширину канала, и никакой длины пароля это
     * не лечит: подбирают не длинный, а чужой короткий. В книге при этом
     * лежат данные, за закрытость которых хозяйства отвечают перед
     * покупателями, — и чужой вход означает не «увидели лишнее», а доступ
     * к правке чужого стада.
     *
     * ## Почему именно столько
     *
     * Десять попыток — граница, за которой человек уже не «ошибся
     * раскладкой», а перебирает. Живой пользователь в такую блокировку
     * попадает редко, а попав, ждёт четверть часа: неприятно, но это
     * не потеря доступа — пароль сбрасывается письмом, и путь этот
     * работает независимо.
     *
     * Пятнадцать минут выбраны из арифметики нападающего, а не из чувства
     * меры: они превращают миллион попыток примерно в тридцать лет.
     * Меньше — заслон перестаёт мешать; больше — блокировка начинает
     * работать против своего же пользователя, и злоумышленнику становится
     * выгодно запирать чужие учётные записи нарочно.
     *
     * ## Чего это не заменяет
     *
     * Счёт ведётся по учётной записи, а не по адресу, откуда пришли.
     * Перебор одного пароля по тысяче разных учётных записей — «обратный
     * перебор» — эта настройка не остановит; для него нужен предел
     * на источник, а он ставится не здесь.
     */
    maxLoginAttempts: 10,
    lockTime: 15 * 60 * 1000,
  },
  ...
  fields: [
    ...
    },
    {
      ...
      defaultValue: false,
      /*
       * Подтверждает Ассоциация — и на создании тоже, иначе им подтверждали
       * себя сами.
       *
       * Последствий у признака нет ни одного: он не участвует ни в правилах
       * доступа, ни в проверках, ни в видимости. Это отметка о разборе
       * заявки для человека, и написать это стоит здесь — плашка в кабинете
       * годами обещала, что до подтверждения данные закрыты, а закрывал
       * их совсем другой флажок.
       */
      access: { create: isAdminField, update: isAdminField },
    },
    {
